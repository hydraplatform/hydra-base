...
class TestTemplateInheritance:
    ...
    def test_add_child_template(self, client):
        ...
        assert (
            template_j.id
            == child_template_j.parent_id
            == child_template_received.parent_id
        )
    ...
    def test_add_type_to_child(self, client):
        ...
        assert (
            len(child_template_received.templatetypes)
        ...
        assert (
            len(parent_template_received.templatetypes)
    ...
    def test_update_type_in_child(self, client):
        ...
        child_type = client.add_child_templatetype(
            type_to_update.id, child_template_j.id
        )
    ...
    def test_add_typeattr_to_child(self, client):
        ...
        newtypeattr = JSONObject({"attr_id": newattr.id, "type_id": child_type.id})
    ...
    def test_add_typeattr_to_child_then_parent(self, client):
        ...
        new_child_typeattr = JSONObject(
            {"attr_id": newattr.id, "type_id": child_type.id}
        )
        ...
        new_parent_typeattr = JSONObject(
            {"attr_id": newattr.id, "type_id": type_to_update.id}
        )
        ...
        assert len(set([ta.attr_id for ta in updated_child_type_attrs])) == len(
            [ta.attr_id for ta in updated_child_type_attrs]
        )
        assert len(set([ta.attr_id for ta in updated_parent_type_attrs])) == len(
            [ta.attr_id for ta in updated_parent_type_attrs]
        )
    ...
    def test_update_child_typeattr(self, client):
        ...
        child_typeattr = client.add_child_typeattr(
            typeattr_to_update.id, child_template_j.id
        )
    ...
    def test_create_network_with_child_template(self, client):
        ...
        parent_template_j = client.testutils.create_template()
        parent_network_type = list(
            filter(
                lambda x: x.resource_type == "NETWORK", parent_template_j.templatetypes
            )
        )[0]
        ...
        child_network_type = list(
            filter(
                lambda x: x.resource_type == "NETWORK", child_template_j.templatetypes
            )
        )[0]
        ...
        node_type = list(
            filter(lambda x: x.resource_type == "NODE", child_template_j.templatetypes)
        )[0]
        ...
        project_j = client.add_project(
            JSONObject({"name": "Template Inheritance Project"})
        )
        ...
        network = JSONObject(
            {
                ...
                "types": [
                    {
                        "id": child_network_type.id,
                        "child_template_id": child_template_j.id,
                    }
                ],
                "nodes": [
                    {"name": "Node1", "x": 0, "y": 0, "types": [{"id": node_type.id}]}
                ],
            }
        )
        ...
        assert (
            requested_network.nodes[0].types[0].child_template_id == child_template_j.id
        )
    ...
    def test_delete_parent_type(self, client):
        ...
        parent_type = list(
            filter(lambda x: x.resource_type == "NODE", parent_template_j.templatetypes)
        )[0]
        ...
        assert (
            len(updated_parent_template_j.templatetypes)
            == len(parent_template_j.templatetypes) - 1
        )
        assert child_type.id not in [
            t.id for t in updated_parent_template_j.templatetypes
        ]

        updated_child_template_j = client.get_template(child_template_j.id)
        assert (
            len(updated_child_template_j.templatetypes)
            == len(child_template_j.templatetypes) - 1
        )
        assert child_type.id not in [
            t.id for t in updated_child_template_j.templatetypes
        ]
    ...
    def test_get_child_template_by_name(self, client):
        ...
        assert (
            len(parent_template_j.templatetypes)
            == len(child_template_j.templatetypes)
            == len(child_template_2_j.templatetypes)
        )
