...
class TestTemplates:
    ...
    def test_add_template(self, client, mock_template):
        ...
        link_attr_1 = client.testutils.create_attribute(
            "link_attr_1", dimension="Pressure"
        )
        link_attr_2 = client.testutils.create_attribute(
            "link_attr_2", dimension="Speed"
        )
        node_attr_1 = client.testutils.create_attribute(
            "node_attr_1", dimension="Volume"
        )
        node_attr_2 = client.testutils.create_attribute(
            "node_attr_2", dimension="Speed"
        )

        new_template_j = mock_template

        assert new_template_j.name == mock_template.name, "Names are not the same!"
        assert json.dumps(new_template_j.layout) == json.dumps(
            mock_template.layout
        ...
        assert (
            len(new_template_j.templatetypes) == 3
        ), "Resource types did not add correctly"
    ...
    def test_update_template(self, client):
        ...
        # Defining attributes
        attribute_1 = client.testutils.create_attribute(
            "link_attr_1", dimension="Pressure"
        )
        attribute_2 = client.testutils.create_attribute(
            "link_attr_2", dimension="Speed"
        )
        attribute_3 = client.testutils.create_attribute(
            "node_attr_1", dimension="Volume"
        )
        ...
        assert (
            len(new_template_j.templatetypes) == 2
        ), "Resource types did not add correctly"
        assert (
            len(new_template_j.templatetypes[0].typeattrs) == 1
        ), "Resource type attrs did not add correctly"
        assert (
            new_template_j.templatetypes[0].typeattrs[0].unit_id
            == client.get_unit_by_abbreviation("bar").id
        )
        ...
        assert (
            updated_template_retrieved_j.name == template.name
        ...
        assert (
            updated_type.name == "Test type 3"
        ), "Resource types did not update correctly"

        assert (
            len(updated_type.typeattrs) == 2
        ), "Resource type template attr did not update correctly"
        ...
        """
        old_attribute_type_to_restore = (
            updated_template_j.templatetypes[0].typeattrs[0].unit_id
        )
        updated_template_j.templatetypes[0].typeattrs[0].unit_id = (
            client.get_unit_by_abbreviation("m^3").id
        )
        with pytest.raises(HydraError):
            client.update_template(updated_template_j)
        # Restoring the old value to not fail later for this reason
        updated_template_j.templatetypes[0].typeattrs[
            0
        ].unit_id = old_attribute_type_to_restore

        """
            Testing the case in which the attribute has no dimension while the typeattr has UNIT_ID that is not none
        """
        attribute_dimension_none = client.testutils.create_attribute(
            "node_attr_dimension_none", dimension=None
        )
        ...
        with pytest.raises(HydraError):
            client.update_template(
                updated_template_j
            )  # It MUST fail because the unit_id is not consistent to dimension_id none
    ...
    def test_update_type(self, client, mock_template):
        ...
        assert (
            len(type_to_update_j.typeattrs) == 2
        ), "Resource type attrs did not add correctly"
        ...
        assert (
            type_to_update_j.alias == updated_type_j.alias
        ), "Aliases are not the same!"
        ...
        assert (
            type_to_update_j.typeattrs[-1].properties["update_type_test_property"]
            == "property value"
        )

        assert (
            len(updated_type_j.typeattrs) == 3
        ), "Template type attrs did not update correctly"
    ...
    def test_add_typeattr(self, client, mock_template):
        ...
        tattr_3.description = (
            "Description of additional typeattr from test_add_typeattr"
        )
        ...
        assert (
            eval(updated_type.typeattrs[-1].properties)["add_typeattr_test_property"]
            == "property value"
        )
    ...
    def test_delete_typeattr(self, client, mock_template):
        ...
        assert (
            len(updated_type.typeattrs) == 1
        ), "Resource type attr did not add correctly"
    ...
    def test_get_template(self, client, mock_template):
        ...
        assert (
            new_template.name == mock_template.name
        ), "Names are not the same! Retrieval by ID did not work!"
    ...
    def test_get_template_by_name_good(self, client, mock_template):
        ...
        assert (
            new_template.name == mock_template.name
        ), "Names are not the same! Retrieval by name did not work!"
    ...
    def test_set_template_status(self, client, network_with_data, mock_template):
        ...
        unfiltered_templates = client.get_templates(
            load_all=False, include_inactive=True
        )
    ...
    def test_add_resource_type(self, client, mock_template):
        ...
        for node in new_net.nodes:
            assert node.types is not None and node.types[0].name == "Node type"
    ...
    def test_find_matching_resource_types(self, client, network_with_data):
        ...
        assert (
            node_to_check.types[0].id in matching_type_ids
        ), "TemplateType ID not found in matching types!"
    ...
    def test_assign_type_to_resource(self, client, mock_template, network_with_data):
        ...
        result = JSONObject(
            client.assign_type_to_resource(templatetype.id, "NODE", node_to_assign.id)
        )
    ...
    def test_remove_type_from_resource(self, client, mock_template, network_with_data):
        ...
        result1_i = client.assign_type_to_resource(
        ...
        assert node_j.types is not None, "Assigning type did not work properly."
    ...
    def test_apply_template_to_network(self, client, mock_template, network_with_data):
        ...
        link_ra_1 = JSONObject(dict(attr_id=link_type.typeattrs[0].attr_id))
        link_ra_2 = JSONObject(dict(attr_id=link_type.typeattrs[1].attr_id))
    ...
    def test_apply_template_to_network_twice(
        self, client, mock_template, network_with_data
    ):
        net_to_update = network_with_data
        template = mock_template
    ...
    def test_remove_template_and_attributes_from_network(
        self, client, mock_template, network_with_data
    ):
        network = network_with_data
        template = mock_template
        ...
        for n in network.nodes:
            ...
            for ra in n.attributes:
                for type_attr in node_type.typeattrs:
                    if (
                        ra.attr_id == type_attr.attr_id
                        and ra.id in rs_ids
                        and type_attr.data_restriction is not None
                    ):
    ...
    def test_validate_network(self, client, network_with_data):
        ...
        # mismatch also.
        errors2 = client.validate_network(
            network.id, networktype.template_id, scenario.id
        )
