...
timeseries_valid_values = [
    {"0": {"1979 Feb 2 0100": 7, "01:00 2 Feb 1979": 9}},
    {"0": {"2012": 12, "2013": 13, "2014": 14}},
    {"0": {"18:00 31 August 1977": 100}},
]
timeseries_invalid_values = [
    "otheriterable",
    list(range(12)),
...
def test_create_array(value):
    array_dataset = hb.lib.objects.Dataset(
        {"type": "array", "value": json.dumps(value)}
    )
...
def make_array(request):
    array_dataset = hb.lib.objects.Dataset(
        {"type": "array", "value": json.dumps(request.param)}
    )
...
def test_create_timeseries(value):
    timeseries_dataset = hb.lib.objects.Dataset(
        {"type": "timeseries", "value": json.dumps(value)}
    )
...
def test_create_dataframe(value):
    dataframe_dataset = hb.lib.objects.Dataset(
        {"type": "dataframe", "value": json.dumps(value)}
    )
...
def test_dataframe_order_preserved():
    ...
    dataframe_dataset = hb.lib.objects.Dataset(
        {"type": "dataframe", "value": df.to_json()}
    )
