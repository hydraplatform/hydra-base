...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers",
        f"{requires_hdf_mark}: Indicates that decorated test requires HDF support",
    )
...
def testdb_uri(db_backend):
    if db_backend == "sqlite":
        ...
        raise ValueError(
            'Database backend "{}" not supported when running the tests.'.format(
                db_backend
            )
        )
...
@pytest.fixture()
def network(
    client, project_id=None, num_nodes=10, new_proj=True, map_projection="EPSG:4326"
):
    return client.testutils.build_network(
        project_id, num_nodes, new_proj, map_projection
    )
...
def network_with_data(
    ...
    return client.testutils.create_network_with_data(
        project_id, num_nodes, ret_full_net, new_proj, map_projection
    )


@pytest.fixture()
def network_with_child_scenario(
    client,
    project_id=None,
    num_nodes=10,
    ret_full_net=True,
    ...
    return client.testutils.create_network_with_child_scenario(
        project_id, num_nodes, ret_full_net, new_proj, map_projection, levels=2
    )


@pytest.fixture()
def network_with_grandchild_scenario(
    client,
    project_id=None,
    num_nodes=10,
    ret_full_net=True,
    ...
    """
    return client.testutils.create_network_with_child_scenario(
        project_id, num_nodes, ret_full_net, new_proj, map_projection, levels=3
    )


@pytest.fixture()
def second_network_with_data(
    client,
    project_id=None,
    num_nodes=10,
    ret_full_net=True,
...
def network_with_extra_group(
    ...
    return client.testutils.create_network_with_extra_group(
        project_id, num_nodes, ret_full_net, new_proj, map_projection
    )
...
def projectmaker(client):
    class ProjectMaker:
        def create(self, name=None, share=True, parent_id=None):
            ...
            return client.testutils.create_project(
...
def attributegroupmaker(client):
    class AttributeGroupMaker:
        def create(self, project_id, name=None, exclusive="N"):
            ...
            newgroup = client.testutils.create_attributegroup(
                project_id, name=name, exclusive=exclusive
            )
