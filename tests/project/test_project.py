...
class TestProject:
    ...
    def add_data(self, client, project):
        ...
        attribute_data.append(
            client.testutils.create_descriptor(attrs[0], val="just project desscriptor")
        )
    ...
    def test_update(self, client, network_with_data):
        ...
        project.description = "A project created through the SOAP interface."
        ...
        assert project_j.id == updated_project.id, "project_id changed on update."
        assert project_j.created_by is not None, "created by is null."
        assert project_j.name == updated_project.name, "project_name changed on update."
        assert (
            project_j.description != updated_project.description
        ...
        assert (
            rs_to_check.dataset.type == "descriptor"
            and rs_to_check.dataset.value == "just project desscriptor"
    ...
    def test_load(self, client):
        ...
        assert new_project.name == project.name, "project_name is not loaded correctly."
        assert (
    ...
    def test_set_project_status(self, client):
        ...
        assert proj.status == "X", "Deleting project did not work correctly."
    ...
    def test_clone_project(self, client, projectmaker, networkmaker):
        ...
        cloned_project_id = client.clone_project(
            proj.id, recipient_user_id=recipient_user.id, new_project_name=None
        )
    ...
    def test_rename_of_status_x_projects_in_clone(
