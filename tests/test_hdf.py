...
def multigroup_file():
    return {
        ...
        "groups": [
            "RFJ_Rffrk_erfhygf",
            "prageny_fbhgu_rffrk_erfhygf",
            "qnvyl_cebsvyrf",
            "yvapbyafuver_erfhygf",
            "zbaguyl_cebsvyrf",
            "gvzrfrevrf",
        ],
    }
...
@pytest.fixture(
    params=["s3://modelers-data-bucket/does_not_exist.h5", "does_not_exist"]
)
def bad_url(request):
...
class TestHdf:
    ...
    def test_private_hdf_no_access(self, hdf, private_aws_file):
        ...
        with pytest.raises(ValueError):
            info = hdf.get_series_info(
                private_aws_file["path"], columns=private_aws_file["series_name"]
            )
    ...
    def test_hdf_dataset(self, hdf, public_aws_file):
        ...
        for series_name, ranges in expected.items():
            for bounds, data in ranges.items():
                df_json = hdf.get_columns_as_dataframe(
                    public_aws_file["path"],
                    columns=[series_name],
                    start=bounds[0],
                    end=bounds[1],
                )
    ...
    def test_hydra_hdf_size(self, client, public_aws_file):
        ...
        """
        assert (
            client.get_hdf_file_size(public_aws_file["path"])
            == public_aws_file["file_size"]
        )
    ...
    def test_hydra_hdf_dataset(self, client, public_aws_file):
        ...
        for series_name, ranges in expected.items():
            for bounds, series in ranges.items():
                df_json = client.get_hdf_columns_as_dataframe(
                    public_aws_file["path"],
                    groupname=None,
                    columns=[series_name],
                    start=bounds[0],
    ...
    def test_bad_series_name(self, client, public_aws_file):
        ...
        """
        with pytest.raises(ValueError):
            info = client.get_hdf_series_info(
                public_aws_file["path"], "nonexistent_series"
            )

        with pytest.raises(ValueError):
            df_json = client.get_hdf_columns_as_dataframe(
                public_aws_file["path"], columns=["nonexistent_series"], start=8, end=16
            )
    ...
    def test_bad_bounds(self, client, public_aws_file):
        ...
        with pytest.raises(ValueError):
            df_json = client.get_hdf_group_as_dataframe(
                public_aws_file["path"], start=-1, end=16
            )

        with pytest.raises(ValueError):
            df_json = client.get_hdf_group_as_dataframe(
                public_aws_file["path"], start=16, end=8
            )

        with pytest.raises(ValueError):
            df_json = client.get_hdf_group_as_dataframe(
                public_aws_file["path"], start=8, end=-1
            )

        with pytest.raises(ValueError):
            df_json = client.get_hdf_group_as_dataframe(
                public_aws_file["path"], start=8, end=1e72
            )
    ...
    def test_multigroup_series(self, client, multigroup_file):
        ...
        NB rot13 strings
        """
        df = client.get_hdf_columns_as_dataframe(
        ...
        assert (
            df[:92]
            == '{"Jbezvatsbeq Vagnxr.Fhccyl.Nzbhag":{"1910-01-01T00:00:00.000":0.0,"1910-01-02T00:00:00.000"'
        )
    ...
    def test_get_hdf_whole_group_as_dataframe(self, client, multigroup_file):
        ...
        """
        df_json = client.get_hdf_group_as_dataframe(
            multigroup_file["path"], groupname="RFJ_Rffrk_erfhygf", start=4, end=8
        )
        assert (
            df_json[:126]
            == '{"Ynatunz Vagnxr.Fhccyl.Nzbhag":{"1910-01-05T00:00:00.000":40.0,'
    ...
    def test_hdf_group_columns(self, client, multigroup_file):
        ...
        NB rot13 strings
        """
        columns = client.get_hdf_group_columns(
            multigroup_file["path"], groupname="RFJ_Rffrk_erfhygf"
        )
    ...
    @pytest.mark.parametrize(
        "pathname, path", {**bad_paths, **{"symlink_path": symlink_path}}.items()
    )
    ...
    @pytest.mark.parametrize(
        "file, file_info",
        [("public_aws_file", public_aws_file), ("multigroup_file", multigroup_file)],
    )
    ...
    def test_index_range(self, client, file, file_info, request):
        try:
            ...
            pass
        groups = client.get_hdf_groups(file_info["path"])
        ir = client.get_hdf_index_range(
            file_info["path"], groupname=groups[0], start=2, end=8
        )
        assert len(ir) > 0
