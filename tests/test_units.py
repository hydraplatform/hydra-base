...
class TestUnits:
    ...
    def test_get_dimension(self, client):
        ...
        assert dimension is not None, "Could not get a dimensions by id."
        assert (
            dimension.name == "Length"
        ), "Could not get the dimension 'Length' by ID 1."
        assert (
            len(dimension.units) > 0
        ...
        assert dimension is not None, "Could not get a dimension by None."
        assert dimension.id is None, "Could not get a dimension by None."
    ...
    def test_get_dimensions(self, client):
        ...
        assert (
            dimension_list is not None and len(dimension_list) != 0
        ), "Could not get the full list of dimensions."
        assert (
            dimension_list[0].id is not None
    ...
    def test_get_dimension_by_name(self, client):
        ...
        assert dimension is not None, "Could not get a dimensions by name."
        assert dimension.id == 1, "Could not get the dimension 1 by aname 'Length'."
        assert (
            len(dimension.units) > 0
        ), "Could not get the dimension units for Dimension ID 1."
    ...
    def test_get_unit(self, client):
        ...
        assert unit is not None, "Could not get a unit by id."
        assert (
            unit.abbreviation == "AU"
    ...
    def test_get_unit_by_abbreviation(self, client):
        ...
        assert unit is not None, "Could not get a unit by abbreviation."
        assert (
            unit.id == 2
    ...
    def test_get_units(self, client):
        ...
        assert (
            units_list is not None and len(units_list) != 0
    ...
    def test_get_dimension_by_unit_measure_or_abbreviation(self, client):
        ...
        assert testdim == resultdim.name, "Getting dimension for 'km' didn't work."

        with pytest.raises(HydraError):
            dimension = client.get_dimension_by_unit_measure_or_abbreviation(
                "not-existing-unit"
            )
    ...
    def test_get_dimension_by_unit_id(self, client):
        ...
        assert dimension is not None, "Could not get a dimension by unit id."
        assert (
            dimension.id == testdim.id
        ), "Could not get the dimension '{}' by unit id {}.".format(
            testdim.name, test_unit_id
        )

        with pytest.raises(HydraError):
            # It must raise an exception
            dimension = client.get_dimension_by_unit_id(
                None, do_accept_unit_id_none=False
            )
    ...
    def test_update_dimension(self, client):
        ...
        testdim = {"id": 1, "name": "Length", "description": "New description"}
        modified_dim = client.update_dimension(testdim)

        assert (
            modified_dim.description == testdim["description"]
    ...
    def test_delete_dimension(self, client):
        ...
        assert (
            len(
                list(filter(lambda x: x["name"] == testdim["name"], old_dimension_list))
            )
            > 0
            and len(
                list(filter(lambda x: x["name"] == testdim["name"], new_dimension_list))
            )
            == 0
    ...
    def test_add_unit(self, client):
        ...
        assert (
            len(
                list(
                    filter(
                        lambda x: x["name"] == new_unit["name"], dimension_loaded.units
                    )
                )
            )
            > 0
        ...
        assert len(unitlist) == 1, "Adding a new unit didn't work as expected"

        assert (
            unitlist[0]["name"] == "Test"
    ...
    def test_update_unit(self, client):
        ...
        assert (
            len(unitlist) > 0 and int(unitlist[0]["cf"]) == 0
    ...
    def test_delete_unit(self, client):
        ...
        assert (
            len(unitlist) > 0 and unitlist[0]["abbreviation"] == testunit.abbreviation
        ...
        assert len(unitlist) == 0, "Deleting unit didn't work correctly."
    ...
    def test_convert_units(self, client):
        ...
        assert result == [0.02], "Converting metres to kilometres didn't work."

        result = client.convert_units([20.0, 30.0, 40.0], "m", "km")
        assert result == [0.02, 0.03, 0.04], "Unit conversion of array didn't work."
    ...
    def test_check_consistency(self, client):
        ...
        assert result1 is True, "Unit consistency check didn't work."
        assert result2 is False, "Unit consistency check didn't work."

    def test_is_global_dimension(self, client):
        result = client.is_global_dimension(1)
        assert result is True, "Is global dimension check didn't work."

    def test_is_global_unit(self, client):
        result = client.is_global_unit(1)
        assert result is True, "Is global unit check didn't work."

    def test_convert_dataset(self, client):
        project = client.testutils.create_project()

        network = client.testutils.create_network_with_data(
            num_nodes=2, project_id=project.id
        )
        ...
        # calculate the sum.
        assert (
            sum(new_val) - sum(old_val_conv) < 0.00001
    ...
    def test_apply_unit_to_network_rs(self, client, network_with_data):
        ...
        old_unit_rs = [
            rs
            for rs in network_with_data.scenarios[0].resourcescenarios
