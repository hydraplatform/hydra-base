...
class TestMongo():
    ...
    def test_add_datasets(self, client, mongo_config, mongo):
        ...
        added = []
        for ds in datasets:
            added.append(client.add_dataset(ds, flush=True))
    ...
    def test_shrink_dataset(self, client, mongo_config, mongo):
        ...
        server_ds = client.add_dataset(ds, flush=True)
        ...
        server_ds.value = json.dumps([random.uniform(1, 100) for _ in range(10)])
        shrunk_ds = client.update_dataset(server_ds)
    ...
    def test_grow_dataset(self, client, mongo_config, mongo):
        ...
        data_sz = mongo_threshold * random.randint(1, 3) + 1
        server_ds.value = json.dumps([random.uniform(1, 100) for _ in range(data_sz)])
        grown_ds = client.update_dataset(server_ds)
