...
class TestTimeSeries:
    ...
    def test_get_relative_data_between_times(self, client, network, relative_dataframe):
        ...
        invalid_qry = client.get_vals_between_times(
            val_to_query.id, now, now + datetime.timedelta(minutes=75), "minutes", 5
        )
    ...
    def test_seasonal_timeseries(self, client, network, seasonal_timeseries):
        ...
        jan_val = client.get_val_at_time(
            val_to_query.id, [datetime.datetime(2000, 1, 10, 00, 00, 00)]
        )
        feb_val = client.get_val_at_time(
            val_to_query.id, [datetime.datetime(2000, 2, 10, 00, 00, 00)]
        )
        mar_val = client.get_val_at_time(
            val_to_query.id, [datetime.datetime(2000, 3, 10, 00, 00, 00)]
        )
        oct_val = client.get_val_at_time(
            val_to_query.id, [datetime.datetime(2000, 10, 10, 00, 00, 00)]
        )
    ...
    def test_multiple_vals_at_time(
        self, client, network_with_data, seasonal_timeseries
    ):
    ...
    def test_descriptor_get_data_between_times(self, client, network_with_data):
        ...
        value = client.get_vals_between_times(
            val_to_query.id, now, now + datetime.timedelta(minutes=75), "minutes", 1
        )
...
def collection_json_object():
    collection = JSONObject(dict(type="descriptor", name="Test collection"))
    return collection
...
class TestDataCollection:
    ...
    def test_add_collection(self, client, network_with_dataset_collection):
        ...
        assert (
            len(collection["items"]) == 2
        ), "Dataset collection does not have any items!"
    ...
    def test_add_dataset_to_collection(
        self, client, network_with_data, collection_json_object
    ):
        ...
        assert (
            client.check_dataset_in_collection(
                dataset_id_to_add, newly_added_collection.id
            )
            == "N"
        )
        assert (
            client.check_dataset_in_collection(99999, newly_added_collection.id) == "N"
        )
        ...
        assert set(new_dataset_ids) - set(previous_dataset_ids) == set(
            [dataset_id_to_add]
        )

    def test_add_datasets_to_collection(
        self, client, network_with_data, collection_json_object
    ):
    ...
    def test_remove_dataset_from_collection(
        self, client, network_with_data, collection_json_object
    ):
    ...
    def test_delete_dataset_thats_in_a_collection(
        self, client, network_with_data, collection_json_object
    ):
    ...
    def test_replace_hidden_data(self, client, network_with_data):
        ...
        for d in netA.scenarios[0].resourcescenarios:
            if d.resource_attr_id == attr_to_be_changed:
                ...
                ts_val = {0: {t1: [11, 21, 31, 41, 51], t2: [12, 22, 32, 42, 52]}}
