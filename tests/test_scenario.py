...
class TestScenario:
    ...
    def get_scenario(self, scenario_id, get_parent_data=False):
        ...
        """
        return JSONObject(
            client.get_scenario(scenario_id, get_parent_data=get_parent_data)
        )
    ...
    def test_get_scenario(self, client, network_with_data):
        ...
        assert len(scenario_retrieved.resourcegroupitems) == len(
        ...
        assert len(scenario_no_data.resourcegroupitems) == len(
            scenario.resourcegroupitems
        )

        scenario_no_groupitems = client.get_scenario(
            scenario.id, include_group_items=False
        )
        assert len(scenario_no_groupitems.resourcescenarios) == len(
    ...
    def test_get_scenario_by_name(self, client, network_with_data):
        ...
        scenario_no_data = client.get_scenario_by_name(
            network.id, scenario.name, include_data=False
        )
        assert scenario_no_data.resourcescenarios == []
        assert len(scenario_no_data.resourcegroupitems) == len(
            scenario.resourcegroupitems
        ...
        scenario_no_groupitems = client.get_scenario_by_name(
            network.id, scenario.name, include_group_items=False
        )
        assert len(scenario_no_groupitems.resourcescenarios) == len(
            scenario.resourcescenarios
        )
    ...
    def test_update(self, client, network_with_data):
        ...
        new_resource_scenario = client.add_data_to_attribute(
            scenario_id, resource_attr_id, dataset
        )

        assert (
            new_resource_scenario.dataset.value == "I am an updated test!"
        ), "Value was not updated correctly!!"
    ...
    def test_add_scenario(self, client, network_with_data, dateformat):
        ...
        descriptor = client.testutils.create_descriptor(
            client.testutils.get_by_name("node_attr_a", node_attrs), "new_descriptor"
        )
        timeseries = client.testutils.create_timeseries(
            client.testutils.get_by_name("node_attr_b", node_attrs)
        )

        for r in new_scenario.resourcescenarios:
            if (
                r.resource_attr_id
                == client.testutils.get_by_name("node_attr_a", node_attrs).id
            ):
                r.dataset = descriptor.dataset
            elif (
                r.resource_attr_id
                == client.testutils.get_by_name("node_attr_b", node_attrs).id
            ):
    ...
    def test_get_dataset_scenarios(self, client, network_with_data):
        ...
        dataset_scenarios = [
            JSONObject(s) for s in client.get_dataset_scenarios(dataset_id_to_check)
        ]
        ...
        assert set([scenario.id, new_scenario.id]).issubset(
            set([s.id for s in dataset_scenarios])
        )
    ...
    def test_update_resourcedata(self, client, network_with_data):
        ...
        new_resourcescenarios = [
            JSONObject(rs)
            for rs in client.update_resourcedata(scenario.id, rs_to_update)
        ]
    ...
    def test_update_resourcedata_single_dataset_update_and_delete(
        ...
        new_value = json.dumps(
            {"index": {"2000-01-01": "test", "2000-02-01": "update"}}
        )
    ...
    def test_update_resourcedata_consistency(self, client, network_with_data):
        ...
        scalar = client.testutils.create_descriptor(
            client.testutils.get_by_name("node_attr_a", node1.attributes), 200
        )

        rs_to_update = self._get_rs_to_update(scenario_2, scalar)

        new_resourcescenarios = client.update_resourcedata(scenario_2.id, rs_to_update)
        rs_2_id = None
    ...
    def test_get_attributes_for_resource(self, client, network_with_data):
        ...
        new_node_data = [
            JSONObject(d)
            for d in client.get_attributes_for_resource(
    ...
    def test_bulk_add_data(self, client, dateformat):
        ...
        ts_val = json.dumps({0: {t1: val_1, t2: val_2, t3: val_3}})
    ...
    def test_clone_scenario(self, client, network_with_data):
        ...
        assert (
            len(scenario_diff.resourcescenarios) == 0
        ), "Scenarios should be the same but are not"
        ...
        assert len(new_scenario_with_results.resourcescenarios) > len(
            new_scenario.resourcescenarios
        )

        updated_network = JSONObject(
            client.get_network(new_scenario.network_id, include_data=True)
        )

        assert (
            len(updated_network.scenarios) == 4
        ), "The network should have 4 scenarios!"

        assert (
            len(updated_network.scenarios[1].resourcescenarios) > 0
        ), "Data was not cloned!"
        ...
        for original_scenario in network.scenarios:
            for updated_scenario in network.scenarios:
                if original_scenario.name == updated_scenario.name:
                    scen_2_vals = set(
                        [rs.dataset.value for rs in original_scenario.resourcescenarios]
                    )
                    scen_1_vals = set(
                        [rs.dataset.value for rs in updated_scenario.resourcescenarios]
                    )
    ...
    def test_get_inherited_data(self, client, network_with_child_scenario):
        ...
        assert (
            len(child.resourcescenarios) == 0
        ), "There's data in the child but there shouldn't be"
        ...
        assert len(retrieved_child_scenario.resourcescenarios) == len(
            parent.resourcescenarios
        )

    def test_get_third_level_inherited_data(
        self, client, network_with_grandchild_scenario
    ):
        ...
        assert (
            len(grandchild.resourcescenarios) == 0
        ), "There's data in the child but there shouldn't be"
        ...
        retrieved_grandchild_scenario = client.get_scenario(
            grandchild.id, get_parent_data=True
        )
        assert len(retrieved_grandchild_scenario.resourcescenarios) == len(
            parent.resourcescenarios
        )
        ...
        inherited_group_items = client.get_resourcegroupitems(
            group_id, grandchild.id, get_parent_items=True
        )
    ...
    def test_inherited_get_scenario_data(self, client, network_with_child_scenario):
        ...
        inherited_scenario_data = client.get_scenario_data(
            child.id, get_parent_data=True
        )
    ...
    def test_inherited_get_resource_data(self, client, network_with_child_scenario):
        ...
        child_resource_data = client.get_resource_data(
            "NODE", resource_to_query.id, child.id
        )

        assert len(child_resource_data) == 0

        parent_resource_data = client.get_resource_data(
            "NODE", resource_to_query.id, parent.id
        )

        inherited_resource_data = client.get_resource_data(
            "NODE", resource_to_query.id, child.id, get_parent_data=True
        )
    ...
    def test_inherited_add_data_to_grandchild(
        self, client, network_with_grandchild_scenario
    ):
        """
        ...
        assert len(child_scenario.resourcescenarios) == len(
            grandchild_scenario.resourcescenarios
        )
        ...
        grandchild_scenario_data = client.get_scenario_data(
            grandchild.id, get_parent_data=True
        )
    ...
    def test_compare(self, client, network_with_data):
        ...
        scenario_diff = JSONObject(
            client.compare_scenarios(scenario.id, new_scenario.id)
        )
        ...
        scenario_diff = JSONObject(
            client.compare_scenarios(scenario_1.id, scenario_2.id)
        )

        # print "Comparison result: %s"%(scenario_diff)

        assert (
            len(scenario_diff.resourcescenarios) == 1
        ), "Data comparison was not successful!"
        ...
        assert (
            len(scenario_diff.groups.scenario_2_items) == 1
        ), "Group comparison was not successful!"
        assert (
            scenario_diff.groups.scenario_1_items == []
        ), "Group comparison was not successful!"
    ...
    def test_delete_resource_scenario(self, client, network_with_data):
        ...
        client.delete_resource_scenario(
            scenario.id, rs_to_delete.resource_attr_id, quiet=True
        )
    ...
    def test_lock_scenario(self, client, network_with_data):
        ...
        updated_ds = JSONObject(
            client.update_dataset(
                ds.id, ds.name, ds.type, ds.value, ds.unit_id, ds.metadata
            )
        )
        ...
        assert (
            locked_resource_scenarios_value.hash
            != unlocked_resource_scenarios_value.hash
        )
    ...
    def test_copy_data_from_scenario(self, client, network_with_data):
        ...
        scenario_diff = JSONObject(
            client.compare_scenarios(source_scenario_id, cloned_scenario.id)
        )
        ...
        client.copy_data_from_scenario(
            [resource_attr_id], cloned_scenario.id, source_scenario_id
        )
        ...
        assert (
            len(scenario_diff.resourcescenarios) == 0
        ), "Scenario update was not successful!"
    ...
    def test_merge_scenario(self, client, networkmaker):
        ...
        source_rs = sorted(
            source_scenario.resourcescenarios, key=lambda x: x.resource_attr_id
        )
        target_rs = sorted(
            target_scenario.resourcescenarios, key=lambda x: x.resource_attr_id
        )
        differences = []
        ...
        merged_scenario = client.get_scenario(
            sorted(updated_target_network.scenarios, key=lambda x: x.cr_date)[-1].id
        )
        source_scenario = client.get_scenario(source_scenario.id)
        source_rs = sorted(
            source_scenario.resourcescenarios, key=lambda x: x.resource_attr_id
        ...
        merged_rs = sorted(
            merged_scenario.resourcescenarios, key=lambda x: x.resource_attr_id
        )
        differences = []
        for i, rs in enumerate(source_rs):
    ...
    def test_set_resourcescenario_dataset(self, client, network_with_data):
        ...
        new_ds = client.add_dataset(
            dataset.type, dataset.value, dataset.unit_id, {}, dataset.name, flush=True
        )
    ...
    def test_add_data_to_attribute(self, client, network_with_data):
        ...
        updated_resource_scenario = client.add_data_to_attribute(
            scenario_id, resource_attr_id, dataset
        )

        new_resource_scenario = client.add_data_to_attribute(
            scenario_id, empty_ra.id, dataset
        )

        assert (
            updated_resource_scenario.dataset.value == "I am an updated test!"
        ), "Value was not updated correctly!!"
        assert (
            new_resource_scenario.dataset.value == "I am an updated test!"
        ), "Value was not updated correctly!!"
