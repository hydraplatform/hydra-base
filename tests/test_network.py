...
class TestNetwork:
    ...
    def test_get_network_with_template(self, client, network_with_data):
        ...
        filtered_net_with_extra_attributes = client.get_network(
            net.id, template_id=template_id, include_non_template_attributes=True
        )

        assert (
            len(filtered_net.attributes)
            == len(filtered_net_with_extra_attributes.attributes) - 2
        )
    ...
    def test_get_all_resource_attributes_in_network(self, client, network_with_data):
        ...
        all_network_resource_attrs = client.get_all_resource_attributes_in_network(
            test_attr_id, network_with_data.id
        )
    ...
    def test_get_network_1(self, client, networkmaker):
        ...
        partial_network = client.get_network(
            new_scenario.network_id,
        ...
        network_with_results = client.get_network(
            new_scenario.network_id,
            include_attributes=True,
            include_data=True,
            scenario_ids=scen_ids,
        )
        network_no_results = client.get_network(
            new_scenario.network_id,
            include_attributes=True,
            include_data=True,
        ...
        assert (
            len(network_with_results.scenarios[0].resourcescenarios)
            == len(network_no_results.scenarios[0].resourcescenarios) + 10
        )
        metadata = (
            json.loads(sample_rs.dataset.metadata)
            if isinstance(sample_rs.dataset.metadata, str)
            else sample_rs.dataset.metadata
        )
        assert len(metadata) == 0

        network_with_results_and_metadata = client.get_network(
            new_scenario.network_id,
            include_attributes=True,
            include_data=True,
            scenario_ids=scen_ids,
            template_id=None,
            include_non_template_attributes=None,
    ...
    def test_update_network(self, client, network_with_data):
        ...
        net.description = "A different network for SOAP unit tests."

        updated_network = client.update_network(net)

        assert net.id == updated_network.id, "network_id has changed on update."
        assert net.name == updated_network.name, "network_name changed on update."
    ...
    def test_add_link(self, client, projectmaker, template):
        ...
        assert len(new_network.links) == len(network.links) + 1
    ...
    def test_add_nodes(self, client, projectmaker):
        ...
        assert len(network.nodes) + len(nodes) == len(new_network.nodes)
        "new nodes were not added correctly_2",
    ...
    def test_get_all_resource_data(self, client, network_with_data):
        ...
        truncated_resource_data = client.get_all_resource_data(
            s.id, include_values=True, include_metadata=True, page_start=0, page_end=1
        )
    ...
    def test_get_resource_data(self, client, network_with_data):
        ...
        all_resource_data = client.get_resource_data(
            "NODE", node.id, s.id, include_values=False
        )
        all_node_types = []
        for rd in all_resource_data:
        ...
        for datatype in all_node_types:
            ...
            all_resource_data = client.get_resource_data(
                "NODE", node.id, s.id, include_values=True
            )
            for rd in all_resource_data:
    ...
    def test_get_all_network_owners(self, client, projectmaker, networkmaker):
        proj = projectmaker.create("test")

        net1 = networkmaker.create(project_id=proj.id)
        client.share_network(net1.id, ["UserB"], "N", "Y")  # Not an admin
        ...
        assert networkowners[0].created_by == pytest.root_user_id
        assert networkowners[0].user_id == pytest.root_user_id

        assert networkowners[1].created_by == pytest.root_user_id
        assert networkowners[1].user_id == pytest.user_b.id
    ...
    def test_clone_network_into_existing_project(self, client, network_with_data):
        ...
        cloned_network_id = client.clone_network(
            net.id,
        ...
        assert (
            ...
            == len(cloned_network.scenarios[0].resourcescenarios) + 10
        )  # this ignores results
        assert len(net.scenarios[0].resourcegroupitems) == len(
            cloned_network.scenarios[0].resourcegroupitems
        )
    ...
    def test_clone_network_with_scoped_attributes(self, client, network_with_data):
        ...
        network_scoped_resource_attributes = list(
            filter(
                lambda x: x.attr_id == network_scoped_attributes[0].id, net.attributes
            )
        )
        ...
        project_scoped_attributes = client.get_attributes(
            project_id=cloned_network.project_id
        )
    ...
    def test_clone_network_with_scoped_attributes_to_new_project(
    ...
    def test_clone_node(self, client, network_with_data):
        ...
        original_node_data = list(
            filter(
                lambda x: x.resource_attr_id
                in [a.id for a in node_to_clone.attributes],
                scenario.resourcescenarios,
            )
        )

        cloned_node_data = list(
            filter(
                lambda x: x.resource_attr_id in [a.id for a in cloned_node.attributes],
                scenario.resourcescenarios,
            )
        )

        assert (
            len(cloned_node_data) == len(original_node_data) - 1
        )  # has no outputs, so has one less dataset
        ...
        cloned_node_data = list(
            filter(
                lambda x: x.resource_attr_id
                in [a.id for a in cloned_node_2.attributes],
                scenario.resourcescenarios,
            )
        )
        ...
        with pytest.raises(hb.exceptions.HydraError):
            cloned_node_id_3 = client.clone_node(
                node_to_clone.id, name=network_with_data.nodes[1].name
            )
        ...
        y = -100
        cloned_node_id_3 = client.clone_node(
            node_to_clone.id, name=name, new_x=x, new_y=y
        )
    ...
    def test_clone_nodes(self, client, network_with_data):
        ...
        for i, cloned_node_id in enumerate(cloned_node_ids):
            ...
            cloned_node_data = list(
                filter(
                    lambda x: x.resource_attr_id
                    in [a.id for a in cloned_node.attributes],
                    scenario.resourcescenarios,
                )
            )
