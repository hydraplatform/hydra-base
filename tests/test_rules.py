...
class TestRules:
    ...
    def add_rule(
        ...
        new_rule_j = JSONObject(
            client.add_rule(scenario_id=None, rule=rule, include_network_users=True)
        )
    ...
    def test_add_rule_type_definition(self, client):
        client.add_rule_type_definition(
            JSONObject({"name": "A new Rule", "code": "a_new_rule"})
        )
    ...
    def test_get_rule_type_definitions(self, client):
        ...
        client.add_rule_type_definition(
            JSONObject({"name": "A new Rule 1", "code": "a_new_rule_1"})
        )
    ...
    def test_clone_network_with_rules(self, client, network_with_data):
        ...
        node_rule_no_scenario_j = self.add_rule(
            client,
            network_with_data,
            ref_key="NODE",
        ...
        node_rule_with_scenario_j = self.add_rule(
            client,
            network_with_data,
            ref_key="NODE",
            ref_id=sorted(network_with_data.nodes, key=lambda x: x.name)[0].id,
        ...
        # sanity check
        assert (
            node_rule_with_scenario_j.scenario_id == network_with_data.scenarios[0].id
        )
        ...
        node_rules = sorted(
            client.get_resource_rules("NODE", sorted_nodes[0].id), key=lambda x: x.value
        )
    ...
    def test_get_rules_by_type(self, client, network_with_data):
        ...
        self.add_rule(
            client, network_with_data, name="Test1", types=[ruletype_A_j.code]
        )
        self.add_rule(
            client, network_with_data, name="Test2", types=[ruletype_A_j.code]
        )
        self.add_rule(
            client,
            network_with_data,
        ...
        self.add_rule(
            client, network_with_data, name="Rule Type B", types=[ruletype_B_j.code]
        )
    ...
    def test_add_rule1(self, client, network_with_data):
        ...
        new_rule_j = self.add_rule(
            client, network_with_data, name=rulename, text=ruletext
        )
    ...
    def test_update_rule(self, client, network_with_data):
        ...
        client.add_rule_type_definition(
            JSONObject({"name": "A new Rule", "code": typecode})
        )
        client.add_rule_type_definition(
            JSONObject({"name": "A new Rule", "code": typecode1})
        )
        ...
        new_rule_j = self.add_rule(
            client, network_with_data, name=rulename, text=ruletext, types=[typecode]
        )

        new_rule_j = client.get_rule(
            new_rule_j.id
        )  # do this to get all the DB server defaults
    ...
    def test_add_rule_to_scenario(self, client, network_with_data):
        ...
        new_rule_a_j = self.add_rule(
            client, network_with_data, name="Added Rule", text="e=mc^3"
        )
        new_rule_b_j = self.add_rule(
            client, network_with_data, name="Added Rule 1", text="e=mc^4"
        )

        new_rule_c_j = self.add_rule(
            client,
            network_with_data,
            scenario_id=scenario_id,
            name="Added Rule 2",
        ...
        resource_rules_with_scenario = client.get_resource_rules(
            "NETWORK", network_with_data.id, scenario_id=scenario_id
        )

        assert len(scenario_rules) == 1
        assert scenario_rules[0].name == new_rule_c_j.name
        assert len(resource_rules) >= 3
        assert {r.name for r in (new_rule_a_j, new_rule_b_j, new_rule_c_j)}.issubset(
            {r.name for r in resource_rules}
        )
    ...
    def test_add_rule_to_node(self, client, network_with_data):
        ...
        resource_rules_with_scenario = client.get_resource_rules(
            "NODE", node_id, scenario_id=scenario_id
        )

        assert len(scenario_rules) == 1
        assert scenario_rules[0].name == new_rule_c_j.name
        assert len(all_resource_rules) >= 3
        assert sorted([r.name for r in all_resource_rules]) == sorted(
            [r.name for r in (new_rule_a_j, new_rule_b_j, new_rule_c_j)]
        )
        assert len(resource_rules_with_scenario) == 1
        assert resource_rules_with_scenario[0].name == new_rule_c_j.name
    ...
    def test_delete_rule(self, client, network_with_data):
        ...
        assert new_rule_j.id in [
            r.id for r in client.get_resource_rules("NETWORK", network_with_data.id)
        ]

        client.delete_rule(new_rule_j.id)

        assert new_rule_j.id not in [
            r.id for r in client.get_resource_rules("NETWORK", network_with_data.id)
        ]
