...
class TestScopedAttribute:
    ...
    def test_add_network_and_project_scoped_attribute(self, client, network_with_data):
        ...
        project_and_network_scoped_attributes = client.get_attributes(
            project_id=network_with_data.project_id, network_id=network_with_data.id
        )
        ...
        # It's 4 because there is one added by default to all new networks and projects, plus the ones we just added
        assert (
            len(global_and_project_and_network_scoped_attributes)
            == len(all_global_attributes) + 4
        )
    ...
    def test_add_duplicate_scoped_attribute(self, client, network_with_data):
        ...
        """
        global_attr = JSONObject({"name": f"Duplicate Attribute", "dimension_id": None})
    ...
    def test_add_network_scoped_attribute_without_ownership(
    ...
    def test_add_project_scoped_attribute_without_ownership(
        self, client, network_with_data
    ):
        """
        Test to ensure only the owner of a network can add ann attribute scoped
    ...
    def test_search_attribute_in_sub_project(self, client, projectmaker, networkmaker):
        ...
        """
        client.user_id = (
            1  # force current user to be 1 to avoid potential inconsistencies
        )
    ...
    def test_rescope_attribute(self, client, projectmaker, networkmaker):
        ...
        net_scoped_attr = client.add_attribute(
            {"network_id": proj1net1.id, "name": "test_scoped_attr"}
        )
        childproj_scoped_attr = client.add_attribute(
            {"project_id": childproj.id, "name": "test_scoped_attr"}
        )
        # safety net to make sure it's been added properly
        net_scoped_attr = client.get_attribute_by_id(attr_id=net_scoped_attr.id)
        childproj_scoped_attr = client.get_attribute_by_id(
            attr_id=childproj_scoped_attr.id
        )
        ...
        new_network_ra = client.add_resource_attribute(
            "NODE", proj1net1.nodes[0].id, net_scoped_attr.id, is_var=False
        )
        new_child_project_ra = client.add_resource_attribute(
            "NODE", childprojnet1.nodes[0].id, childproj_scoped_attr.id, is_var=False
        )

        # now add an attribute with the same name at the project level, thus creating a potential
        # duplication
        newly_scoped_attr = client.add_attribute(
            {"project_id": proj1.id, "name": "test_scoped_attr"}
        )
        ...
        updated_child_project_ra = client.get_resource_attribute(
            new_child_project_ra.id
        )

        assert updated_network_ra.attr_id == newly_scoped_attr.id
        assert updated_child_project_ra.attr_id == newly_scoped_attr.id

        matching_attributes = client.search_attributes(
            "test_scoped", network_id=proj1net1.id
        )
        ...
        proj2_net_scoped_attr_ = client.add_attribute(
            {"network_id": proj2net1.id, "name": "test_scoped_attr"}
        )
        # safety net to make sure it's been added properly
        proj2_net_scoped_attr = client.get_attribute_by_id(
            attr_id=proj2_net_scoped_attr_.id
        )
        new_ra = client.add_resource_attribute(
            "NODE", proj2net1.nodes[0].id, proj2_net_scoped_attr.id, is_var=False
        )
        assert proj2_net_scoped_attr.network_id == proj2net1.id

        # now add an attribute with the same name at the project level, thus creating a potential
        # duplication
        proj2_newly_scoped_attr = client.add_attribute(
            {"project_id": proj2.id, "name": "test_scoped_attr"}
        )
        ...
        proj2_scoped_attr = client.get_attribute_by_id(
            attr_id=proj2_newly_scoped_attr.id
        )
        assert proj2_scoped_attr != newly_scoped_attr

    def test_different_projects_same_name_attributes(
        self, client, projectmaker, networkmaker
    ):
        """
        ...
        project1_scoped_attr = JSONObject(
            {"name": f"Project Attribute", "dimension_id": None, "project_id": proj1.id}
        )

        project2_scoped_attr = JSONObject(
            {"name": f"Project Attribute", "dimension_id": None, "project_id": proj2.id}
        )

        proj1_attr = client.add_attribute(project1_scoped_attr)
        net1 = networkmaker.create(project_id=proj1.id)
        proj1_ra = client.add_resource_attribute(
            "NODE", net1.nodes[0].id, proj1_attr.id, is_var=False
        )
        ...
        proj2_ra = client.add_resource_attribute(
            "NODE", net2.nodes[0].id, proj2_attr.id, is_var=False
        )
    ...
    def test_bulk_add_network_and_project_scoped_attribute(
        self, client, network_with_data
    ):
        """
        Test adding a network-scoped attributes.
        ...
        new_attributes = client.add_attributes(
            [global_attr, network_scoped_attr, project_scoped_attr]
        )
    ...
    def test_add_network_scoped_attribute_with_attribute_already_existing(
        self, client, network_with_data
    ):
        """
        ...
        new_project_attr = list(
            filter(
                lambda x: x.project_id == network_with_data.project_id, new_attributes
            )
        )[0]
