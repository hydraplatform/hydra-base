...
class TestAttribute:
    ...
    def test_add_attribute(self, client):
        test_attr = JSONObject(
            {"name": f"Test Attribute {datetime.datetime.now()}", "dimension_id": None}
        )
        ...
        assert new_attr.name == test_attr.name, "add_attribute didn't work"
    ...
    def test_update_attribute(self, client):
        ...
        assert (
            new_attr.id == updated_attr.id and updated_attr.name == new_attr.name
    ...
    def test_delete_attribute(self, client):
        test_attr = JSONObject({"name": "Test Attribute 1", "dimension_id": None})
        new_attr = client.add_attribute(test_attr)
    ...
    def test_add_attributes(self, client):
        test_attrs = [
            JSONObject({"name": "Test Attribute 1", "dimension_id": None}),
            JSONObject({"name": "Test Attribute 2", "dimension_id": 1}),
        ]
        ...
        assert sorted([a.id for a in new_attrs_list_1]) == sorted(
            [a.id for a in new_attrs_list_2]
        )
    ...
    def test_get_attribute_by_name_and_dimension(self, client, attribute):
        ...
        retrieved_attr = client.get_attribute_by_name_and_dimension(
            existing_attr.name, existing_attr.dimension_id
        )
...
class TestResourceAttribute:
    ...
    def test_get_all_network_attributes(self, client, network_with_data):
        ...
        assert len(set([a.id for a in all_network_attributes])) == len(
            manual_all_network_attributes
        )
    ...
    def test_get_all_group_attributes(self, client, network_with_data):
        ...
        group_attributes = client.get_all_resource_attributes(
            "GROUP", network_with_data.id
        )
    ...
    def test_get_all_link_attributes(self, client, network_with_data):
        ...
        link_attributes = client.get_all_resource_attributes(
            "LINK", network_with_data.id
        )
    ...
    def test_get_all_node_attributes(self, client, network_with_data):
        ...
        node_attributes = client.get_all_resource_attributes(
            "NODE", network_with_data.id
        )
        ...
        node_attributes = client.get_all_resource_attributes(
            "NODE", network_with_data.id, template_id
        )
    ...
    def test_add_network_attrs_from_type(self, client, network_with_data, attribute):
        ...
        new_typeattr = JSONObject({"attr_id": attribute.id})
    ...
    def OLD_test_get_network_attrs(self, client, network_with_data):
        ...
        net_type_attrs = client.get_resource_attributes(
            "NETWORK", network_with_data.id, network_with_data.types[0].id
        )
    ...
    def test_delete_all_duplicate_attributes(self, client, network_with_data):
        ...
        client.add_resource_attribute(
            "NETWORK", network_with_data.id, dupe_attr_1.id, "Y"
        )
        client.add_resource_attribute(
            "NETWORK", network_with_data.id, dupe_attr_2.id, "Y"
        )
        ...
        client.add_typeattr(
            JSONObject({"type_id": templatetype_to_update, "attr_id": dupe_attr_1.id})
        )
        client.add_typeattr(
            JSONObject({"type_id": templatetype_to_update, "attr_id": dupe_attr_2.id})
        )
    ...
    def test_delete_duplicate_resourceattributes(self, client, network_with_data):
        ...
        duplicate_attribute1 = JSONObject(
            {"name": ra_attribute1.name, "dimension_id": 1}
        )
        duplicate_attribute2 = JSONObject(
            {"name": ra_attribute2.name, "dimension_id": 1}
        )

        duplicate_attribute3 = JSONObject(
            {"name": ra_attribute3.name, "dimension_id": 1}
        )
        ...
        dupe_ra1 = client.add_resource_attribute(
            "NETWORK", network_with_data.id, dupeattr1.id, "N"
        )  #
        ...
        new_rscen = client.add_data_to_attribute(
            network_with_data.scenarios[0].id, dupe_ra1.id, dataset
        )
        # add 2 more dupes but with no data associated to them
        dupe_ra2 = client.add_resource_attribute(
            "NETWORK", network_with_data.id, dupeattr2.id, "N"
        )
        dupe_ra3 = client.add_resource_attribute(
            "NETWORK", network_with_data.id, dupeattr3.id, "N"
        )
...
class TestAttributeMap:
    ...
    def test_set_attribute_mapping(self, client, networkmaker):
        ...
        updated_rs = client.update_value_from_mapping(
            attr_1.id, attr_2.id, s1.id, s2.id
        )
...
class TestAttributeGroups:
    ...
    def test_basic_add_attribute_group_items(
        self, client, projectmaker, network_with_data, attributegroupmaker
    ):
        project = projectmaker.create()
        ...
        assert len(all_items_in_network) == len(network_attributes) + len(
            node_attributes
        )

    def test_exclusive_add_attribute_group_items(
        self, client, projectmaker, network_with_data, attributegroupmaker
    ):
        """
    ...
    def test_delete_attribute_group_items(
        self, client, projectmaker, network_with_data, attributegroupmaker
    ):
        project = projectmaker.create()
