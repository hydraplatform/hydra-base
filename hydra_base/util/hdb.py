...
from ..exceptions import (
    HydraError,
    HydraLoginUserNotFound,
    HydraLoginUserMaxAttemptsExceeded,
    HydraLoginUserPasswordWrong,
)
...
def add_resource_attributes(resource_i, attributes):
    ...
    for ra in attributes:
        ...
        if ra.id < 0:
            ...
            ra_i = (
                db.DBSession.query(ResourceAttr).filter(ResourceAttr.id == ra.id).one()
            )
...
def make_root_user():
    try:
        ...
        user = User(username="root", password=root_pwd, display_name="Root User")
    ...
    try:
        userrole = (
            db.DBSession.query(RoleUser)
            .filter(RoleUser.role_id == role.id, RoleUser.user_id == user.id)
            .one()
        )
    except NoResultFound:
...
def login_user(username, password):
    ...
    if get_remaining_login_attempts(username, user_id=user_i.id) <= 0:
        ...
        raise HydraLoginUserMaxAttemptsExceeded(
            "Max login attempts exceeded for user {}".format(username)
        )
    ...
    if bcrypt.checkpw(password, userPassword):
        ...
        log.info(
            "User {} now has {} failed logins".format(
                username, user_i.failed_logins + 1
            )
        )
...
def create_default_users_and_perms():
    ...
    for role_code, perm_code in roleperms:
        ...
        links_found = (
            ...
            .filter(RolePerm.role_id == id_maps_dict["role"][role_code])
            .filter(RolePerm.perm_id == id_maps_dict["perm"][perm_code])
            .all()
        )
...
def create_default_units_and_dimensions(update=True):
    ...
    for json_dimension in d["dimension"]:
        ...
        db_dimensions_by_name = (
            db.DBSession.query(Dimension).filter(Dimension.name == dimension_name).all()
        )
        ...
        for json_unit in json_dimension["unit"]:
            db_units_by_name = (
                ...
                .filter(Unit.abbreviation == get_utf8_encoded_string(json_unit["abbr"]))
                .all()
            )
            if len(db_units_by_name) == 0:
                # Adding the unit
                log.debug(
                    "Adding Unit %s in %s", json_unit["abbr"], json_dimension["name"]
                )
                new_unit = Unit()
                if "id" in json_unit:
                ...
                if "description" in json_unit:
                    ...
                    new_unit.description = get_utf8_encoded_string(
                        json_unit["description"]
                    )
