...
class TestUtil:
    ...
    def create_template(self):
        ...
        node_attr_4 = self.create_attribute(
            "node_attr_d", dimension="Volumetric flow rate"
        )
    ...
    def create_child_template(self, parent_id):
        ...
        node_attr_child = self.create_attribute(
            "node_attr_child", dimension="Volumetric flow rate"
        )
        ...
        if len(user_projects) == 0:
            ...
            if share is True:
                self.client.share_project(
                    project.id, ["UserA", "UserB", "UserC"], "N", "Y"
                )

            proj_scoped_attr = self.create_attribute(
                "Project Scoped Attr", dimension=None, project_id=project.id
            )
    ...
    def build_network(
        self, project_id=None, num_nodes=10, new_proj=True, map_projection="EPSG:4326"
    ):
        ...
        for n in range(num_nodes):
            ...
            if prev_node is not None:
                ...
                link = self.create_link(
                    n * -1, node["name"], prev_node["name"], node["id"], prev_node["id"]
                )
        ...
        grp_timeseries = self.create_timeseries(
            group_attrs[0], "cm^3"
        )  # incorrect unit to test validation
    ...
    def create_network_with_data(
        ...
        network = self.build_network(
            project_id, num_nodes, new_proj=new_proj, map_projection=map_projection
        )
        ...
        net_scoped_attr = self.create_attribute(
            "Network Scoped Attr",
            dimension=None,
            network_id=response_network_summary.id,
        )
        self.client.add_resource_attribute(
            "NETWORK", response_network_summary.id, net_scoped_attr.id, False
        )
        ...
        if ret_full_net is True:
            ...
            start = datetime.datetime.now()
            net = self.client.get_network(
                response_network_summary.id, include_data=True
            )
    ...
    def create_network_with_extra_group(
        self,
        project_id=None,
        num_nodes=10,
        ret_full_net=True,
    ...
    def create_network_with_child_scenario(
        self,
        project_id=None,
        num_nodes=10,
        ret_full_net=True,
        ...
        for level in range(levels - 1):
            new_scenario_j = self.client.create_child_scenario(
                parent_scenario_id, "Scenario {0}".format(scenario_count)
            )
            parent_scenario_id = new_scenario_j.id
            scenario_count = scenario_count + 1

        updated_network = JSONObject(
            self.client.get_network(network.id, include_data=True)
        )
    ...
    def check_network(self, request_net, response_net):
        ...
        ordered_rs_request = sorted(
            s["resourcescenarios"], key=lambda x: x.dataset.value
        )
    ...
    def create_timeseries(self, resource_attr, unit="m^3"):
        ...
        fmt = hydra_base.config.get(
            "DEFAULT", "datetime_format", "%Y-%m-%dT%H:%M:%S.%f000Z"
        )
    ...
    def create_dataframe(
        ...
        if dataframe_value is None:
            ...
            dataframe_value = {
                "test_column": {"key1": val_1, "key2": val_2, "key3": val_3}
            }
    ...
    def create_attributes(self):
        ...
        for my_attr in total_attrs:
            assert (
                len(list(filter(lambda x: x.description == my_attr.description, attrs)))
                > 0
            )
    ...
    def create_attribute(
        self, name="Test attribute", dimension=None, network_id=None, project_id=None
    ):
        dimension_id = None
        if dimension is not None:
            dimension_id = self.client.get_dimension_by_name(dimension).id

        attr_i = self.client.get_attribute_by_name_and_dimension(
