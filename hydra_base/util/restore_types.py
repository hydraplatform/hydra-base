...
class TypeRestorer:
    ...
    def check_inputs(self):
        ...
        if self.type_csv is not None and self.template_id is None:
            raise Exception(
                "Cannot use --type-csv without a --template-id argument."
                " Please specify a --template-id"
            )

        if self.target_network_id is None:
            raise Exception(
                "Please specify a target network ID with '--target_network_id'"
            )
    ...
    def load_data(self):
        ...
        self.target_network = (
            hb.db.DBSession.query(Network)
            .filter(Network.id == self.target_network_id)
            .one()
        )
        self.target_nodes = (
            hb.db.DBSession.query(Node)
            .filter(Node.network_id == self.target_network_id)
            .all()
        )
        self.target_links = (
            hb.db.DBSession.query(Link)
            .filter(Link.network_id == self.target_network_id)
            .all()
        )
        self.target_groups = (
            hb.db.DBSession.query(ResourceGroup)
            .filter(ResourceGroup.network_id == self.target_network_id)
            .all()
        )
    ...
    def set_default_link_type(self):
        ...
        if self.default_link_name is not None:
            ...
            if self.default_link_type is None:
                raise Exception(
                    f"Default Link Type {self.default_link_name} "
    ...
    def load_type_map_from_csv(self):
        ...
        self.template = (
            hb.db.DBSession.query(Template)
            .filter(Template.id == self.template_id)
            .first()
        )
        ...
        for nodename, typename in self.csv_df.itertuples():
            ...
            if typename not in type_name_map:
                raise Exception(
                    f"Type name {typename} not found in template {self.template_id}"
    ...
    def get_network_type(self):
        ...
        raise Exception(
            f"Unable to find NETWORK template type in template {self.template.id}"
        )
    ...
    def load_type_map_from_source_network(self):
        ...
        self.source_network = (
            hb.db.DBSession.query(Network)
            .filter(Network.id == self.source_network_id)
            .one()
        )
        self.source_nodes = (
            hb.db.DBSession.query(Node)
            .filter(Node.network_id == self.source_network_id)
            .all()
        )
        self.source_links = (
            hb.db.DBSession.query(Link)
            .filter(Link.network_id == self.source_network_id)
            .all()
        )
        self.source_groups = (
            hb.db.DBSession.query(ResourceGroup)
            .filter(ResourceGroup.network_id == self.source_network_id)
            .all()
        )
    ...
    def check_networks_compatible(self):
        ...
        if len(incompatible_nodes) > 0:
            print(
                f"***ERROR*** Unable to map types. Found a node name(s) in"
        ...
        if self.default_link_type is None:
            for l in self.target_links:
                ...
                if (start_node, end_node) not in self.link_type_map:
                    print(
                        f"**ERROR** Unable to map types. Found a link name '{l.name}' in"
                        f" target network {self.target_network_id} which is "
        ...
        for g in self.target_groups:
            if g.name not in self.group_type_map:
                print(
                    f"Unable to map types. Found a group name '{g.name}' in"
                    f" target network {self.target_network_id} which is "
                    f"not in the source network {self.source_network_id}"
                )
                return False
    ...
    def run(self):
        ...
        self.add_resource_type("NETWORK", self.target_network, self.get_network_type())
        ...
        for l in self.target_links:
            start_node = l.node_a.name
            end_node = l.node_b.name
            matching_type = self.link_type_map.get(
                (start_node, end_node), self.default_link_type
            )
        ...
        try:
            ...
            if self.debug is not True:
                ...
                print(
                    "**DEBUG MODE ENABLED** Inserted types: %s"
                    % self.new_resource_types
                )
...
@click.option(
    "--debug", is_flag=True, default=False, help="Don't commit the transaction if set."
)
def restore(
    ...
    restorer = TypeRestorer(
        source_network_id,  # either this OR
        type_csv,  # this and...
        template_id,  # ...this
