...
def _update_attributes(resource_i, attributes):
    ...
    if resource_i.ref_key == "NETWORK":
        resource_attribute_qry = resource_attribute_qry.filter(
            ResourceAttr.network_id == resource_i.id
        )
    elif resource_i.ref_key == "NODE":
        resource_attribute_qry = resource_attribute_qry.filter(
            ResourceAttr.node_id == resource_i.id
        )
    elif resource_i.ref_key == "LINK":
        resource_attribute_qry = resource_attribute_qry.filter(
            ResourceAttr.link_id == resource_i.link_id
        )
    elif resource_i.ref_key == "GROUP":
        resource_attribute_qry = resource_attribute_qry.filter(
            ResourceAttr.group_id == resource_i.group_id
        )
...
def get_scenario_by_name(network_id, scenario_name, **kwargs):
    try:
        scen = (
            ...
            .filter(
                and_(
                    ...
                    func.lower(Scenario.id) == scenario_name.lower(),
                )
            )
            .one()
        )
        ...
        log.info("No scenario in network %s with name %s" % (network_id, scenario_name))
        return None
...
def _check_ra_duplicates(all_resource_attrs, resource_id_name_map):
    ...
    for ra in all_resource_attrs:
        ...
        if unique_ra_check.get(k) is None:
            ...
            raise HydraError(
                f"Duplicate Resource Attr specified: {resource_name}  {attr_name}"
            )


def _bulk_add_resource_attrs(
    network_id, ref_key, resources, resource_name_map, template_lookup=None
):
        ...
        if resource.attributes is not None:
            for ra in resource.attributes:
                if attr_lookup.get(ra.attr_id) is None:
                    raise HydraError(
                        f"Unable to process attribute {ra.attr_id} on resource {resource.name} as it does not exist"
                    )
    ...
    logging.info(
        "Resource attributes from resources added in %s", (datetime.datetime.now() - t0)
    )
            ...
            for resource_type in resource.types:
                ...
                if resource_type.child_template_id is None:
                    if type_child_template_id_lookup.get(resource_type_id) is None:
                        if (
                            network_child_template_id is None
                            and checked_for_child_template == False
                        ):
                            network_child_template_id = template.get_network_template(
                                network_id, resource_type.id
                            )  # TODO this should be type_id
                        ...
                        if network_child_template_id is None:
                            tt = template.get_templatetype(
                                resource_type.id, user_id=admin_id
                            )

                            network_child_template_id = tt.template_id

                        type_child_template_id_lookup[resource_type_id] = (
                            network_child_template_id
                        )

                    resource_type.child_template_id = type_child_template_id_lookup[
                        resource_type_id
                    ]
                ...
                if resource_type.id is None:
                    raise HydraError(
                        f"Resource type on resource {resource_i.name} has no ID"
                    )
                ...
                if template_j is None:
                    ...
                    template_j = template.get_template(
                        resource_type.child_template_id, user_id=admin_id
                    )
                    template_lookup[template_j.id] = template_j
                for tt in template_j.templatetypes:
                    typeattr_lookup[tt.id] = tt.typeattrs

                typeattrs = typeattr_lookup.get(
                    resource_type.id, []
                )  # TODO this should be type_id
                for ta in typeattrs:
                    if ta.attr_id not in existing_attrs:
                        resource_attrs[resource.id].append(
                            {
                                ...
                                "link_id": resource_i.id if ref_key == "LINK" else None,
                                "group_id": (
                                    resource_i.id if ref_key == "GROUP" else None
                                ),
                                "network_id": (
                                    resource_i.id if ref_key == "NETWORK" else None
                                ),
                                "attr_id": ta.attr_id,
                        ...
                        if ta.default_dataset_id is not None:
                            defaults[(ref_id, ta.attr_id)] = {
                                "dataset_id": ta.default_dataset_id
                            }

    if len(resource_resource_types) > 0:
        db.DBSession.bulk_insert_mappings(ResourceType, resource_resource_types)
    logging.info(
        "%s ResourceTypes inserted in %s secs",
        len(resource_resource_types),
        str(time.time() - t0),
    )

    logging.info(
        "Resource attributes from types added in %s",
    ...
    logging.info(
        "Resource attributes insertion from types done in %s",
        (datetime.datetime.now() - start_time),
    )
    ...
    if ref_key == "NODE":
        ...
        res_qry = res_qry.join(ResourceGroup).filter(
    ...
    logging.info(
        "retrieved %s entries in %s",
        len(real_resource_attrs),
        (datetime.datetime.now() - start_time),
    )
    ...
    logging.info(
        "Processing Query results took %s", (datetime.datetime.now() - start_time)
    )
    ...
    logging.info(
        "Resource attributes added in %s", (datetime.datetime.now() - start_time)
    )
...
def _add_nodes(net_i, nodes, template_lookup):
    ...
    node_attrs, defaults, template_lookup = _bulk_add_resource_attrs(
        net_i.id, "NODE", nodes, iface_nodes, template_lookup
    )
...
def _add_links(net_i, links, node_id_map, template_lookup):
    ...
    link_attrs, defaults, template_lookup = _bulk_add_resource_attrs(
        net_i.id, "LINK", links, iface_links, template_lookup
    )
...
def _add_resource_groups(net_i, resourcegroups, template_lookup):
    ...
    group_attrs, defaults, template_lookup = _bulk_add_resource_attrs(
        net_i.id, "GROUP", resourcegroups, iface_groups, template_lookup
    )
...
def add_network(network, **kwargs):
    ...
    proj_i = (
        db.DBSession.query(Project).filter(Project.id == network.project_id).first()
    )

    if proj_i is None:
        raise HydraError(
            "Project ID is none. A project ID must be specified on the Network"
        )

    existing_net = (
        db.DBSession.query(Network)
        .filter(Network.project_id == network.project_id, Network.name == network.name)
        .first()
    )

    if existing_net is not None:
        raise HydraError(
            f"A network with the name {network.name} is already"
    ...
    network_attrs, network_defaults, template_lookup = _bulk_add_resource_attrs(
        net_i.id, "NETWORK", [network], name_map
    )
    ...
    node_id_map, node_attrs, node_datasets = _add_nodes(
        net_i, network.nodes, template_lookup
    )
    all_resource_attrs.update(node_attrs)

    link_id_map, link_attrs, link_datasets = _add_links(
        net_i, network.links, node_id_map, template_lookup
    )
    all_resource_attrs.update(link_attrs)

    grp_id_map, grp_attrs, grp_datasets = _add_resource_groups(
        net_i, network.resourcegroups, template_lookup
    )
    all_resource_attrs.update(grp_attrs)

    defaults = (
        list(grp_datasets.values())
        + list(link_datasets.values())
        + list(node_datasets.values())
        + list(network_defaults.values())
    )
        ...
        for s in network.scenarios:
            ...
            for r_scen in s.resourcescenarios:
                if all_resource_attrs.get(r_scen.resource_attr_id) is None:
                    raise HydraError(
                        f"Couldn't find resource attribute {r_scen.resource_attr_id} "
            ...
            datasets = data._bulk_insert_data(
                incoming_datasets, user_id, kwargs.get("app_name")
            )
            ...
            for i, ra in enumerate(scenario_resource_attrs):
                scen.add_resource_scenario(
                    ra, datasets[i], source=kwargs.get("app_name")
                )
            ...
            if s.resourcegroupitems is not None:
                for group_item in s.resourcegroupitems:
                    ...
                    if group_item.ref_key == "NODE":
                        ...
                        raise HydraError(
                            "A ref key of %s is not valid for a "
...
def _get_all_resource_attributes(
    network_id, template_id=None, include_non_template_attributes=False
):
    """
    ...
    all_group_attribute_qry = base_qry.join(ResourceGroup).filter(
        ResourceGroup.network_id == network_id
    )
    ...
    attribute_qry = all_node_attribute_qry.union(
        all_link_attribute_qry, all_group_attribute_qry, network_attribute_qry
    )
    ...
    template_attr_lookup, all_network_typeattrs = (
        _get_network_template_attribute_lookup(network_id)
    )
...
def _get_network_template_attribute_lookup(network_id):
    ...
    network_types = (
        ...
        .filter(ResourceType.network_id == network_id)
        .all()
    )
...
def _get_all_templates(network_id, template_id):
    ...
    base_qry = db.DBSession.query(
        ...
        TemplateType.id == ResourceType.type_id, Template.id == TemplateType.template_id
    )

    all_node_type_qry = base_qry.filter(
        Node.id == ResourceType.node_id, Node.network_id == network_id
    )

    all_link_type_qry = base_qry.filter(
        Link.id == ResourceType.link_id, Link.network_id == network_id
    )

    all_group_type_qry = base_qry.filter(
        ResourceGroup.id == ResourceType.group_id,
        ResourceGroup.network_id == network_id,
    )
    ...
    type_qry = all_node_type_qry.union(
        all_link_type_qry, all_group_type_qry, network_type_qry
    )
...
def _get_nodes(network_id, template_id=None):
    ...
    node_qry = (
        db.DBSession.query(Node)
        .filter(Node.network_id == network_id, Node.status == "A")
        .options(noload(Node.network))
    )
    if template_id is not None:
        node_qry = node_qry.filter(
...
def _get_links(network_id, template_id=None):
    ...
    extras = {"types": [], "attributes": []}
    link_qry = (
        db.DBSession.query(Link)
        .filter(Link.network_id == network_id, Link.status == "A")
        .options(noload(Link.network))
    )
    if template_id is not None:
        link_qry = link_qry.filter(
...
def _get_groups(network_id, template_id=None):
    ...
    extras = {"types": [], "attributes": []}
    group_qry = (
        db.DBSession.query(ResourceGroup)
        .filter(ResourceGroup.network_id == network_id, ResourceGroup.status == "A")
        .options(noload(ResourceGroup.network))
    )

    if template_id is not None:
        group_qry = group_qry.filter(
...
def _get_scenarios(
    ...
    scen_qry = (
        db.DBSession.query(Scenario)
        .filter(Scenario.network_id == network_id)
        .options(noload(Scenario.network))
        .filter(Scenario.status == "A")
    )
    ...
    **kwargs,
    ...
    try:
        ...
        net_i = (
            ...
            .options(noload(Network.scenarios))
            .options(noload(Network.nodes))
            .options(noload(Network.links))
            .options(noload(Network.types))
            .options(noload(Network.attributes))
            .options(noload(Network.resourcegroups))
            .one()
        )
        ...
        if include_attributes in ("Y", True):
            all_attributes = _get_all_resource_attributes(
                network_id, template_id, include_non_template_attributes
            )
...
def get_networks(network_ids, **kwargs):
    ...
    networks = db.DBSession.query(Network).filter(Network.id.in_(network_ids))
...
def get_nodes(network_id, template_id=None, **kwargs):
    ...
    node_qry = (
        ...
        .filter(Node.network_id == network_id, Node.status == "A")
        .options(noload(Node.network))
        .options(joinedload(Node.types).joinedload(ResourceType.templatetype))
        .options(joinedload(Node.attributes).joinedload(ResourceAttr.attr))
    )
    if template_id is not None:
        node_qry = node_qry.filter(
            ResourceType.node_id == Node.id,
...
def get_links(network_id, template_id=None, **kwargs):
    ...
    link_qry = (
        ...
        .filter(Link.network_id == network_id, Link.status == "A")
        .options(noload(Link.network))
        .options(joinedload(Link.types).joinedload(ResourceType.templatetype))
        .options(joinedload(Link.attributes).joinedload(ResourceAttr.attr))
    )
...
def get_groups(network_id, template_id=None, **kwargs):
    ...
    group_qry = (
        ...
        .filter(ResourceGroup.network_id == network_id, ResourceGroup.status == "A")
        .options(noload(ResourceGroup.network))
        .options(joinedload(ResourceGroup.types).joinedload(ResourceType.templatetype))
        .options(joinedload(ResourceGroup.attributes).joinedload(ResourceAttr.attr))
    )
    if template_id is not None:
        group_qry = group_qry.filter(
            ResourceType.group_id == ResourceGroup.id,
...
def get_network_simple(network_id, **kwargs):
    try:
        n = (
            db.DBSession.query(Network)
            .filter(Network.id == network_id)
            .options(joinedload(Network.attributes).joinedload(ResourceAttr.attr))
            .one()
        )
        n.types
        for t in n.types:
...
def get_node(node_id, scenario_id=None, **kwargs):
    ...
    if scenario_id is not None:
        res_scens = scenario.get_resource_data(
            "NODE", node_id, scenario_id, None, **kwargs
        )
        rs_dict = {}
        for rs in res_scens:
...
def get_link(link_id, scenario_id=None, **kwargs):
    ...
    if scenario_id is not None:
        res_scens = scenario.get_resource_data(
            "LINK", link_id, scenario_id, None, **kwargs
        )
        rs_dict = {}
        for rs in res_scens:
            rs_dict[rs.resource_attr_id] = JSONObject(rs)
...
def get_resourcegroup(group_id, scenario_id=None, **kwargs):
    try:
        rg = (
            db.DBSession.query(ResourceGroup)
            .filter(ResourceGroup.id == group_id)
            .options(joinedload(ResourceGroup.attributes).joinedload(ResourceAttr.attr))
            .one()
        )
        rg.types
    ...
    if scenario_id is not None:
        res_scens = scenario.get_resource_data(
            "GROUP", group_id, scenario_id, None, **kwargs
        )
        rs_dict = {}
        for rs in res_scens:
            rs_dict[rs.resource_attr_id] = JSONObject(rs)
...
def get_node_by_name(network_id, node_name, **kwargs):
    try:
        n = (
            db.DBSession.query(Node)
            .filter(Node.name == node_name, Node.network_id == network_id)
            .options(joinedload(Node.attributes).joinedload(ResourceAttr.Attr))
            .one()
        )
        return n
...
def get_link_by_name(network_id, link_name, **kwargs):
    try:
        l = (
            db.DBSession.query(Link)
            .filter(Link.name == link_name, Link.network_id == network_id)
            .options(joinedload(Link.attributes).joinedload(ResourceAttr.attr))
            .one()
        )
        return l
        ...
        raise ResourceNotFoundError(
            "Link %s not found in network %s" % (link_name, network_id)
        )
...
def get_resourcegroup_by_name(network_id, group_name, **kwargs):
    try:
        rg = (
            db.DBSession.query(ResourceGroup)
            .filter(
                ResourceGroup.name == group_name, ResourceGroup.network_id == network_id
            )
            .options(joinedload(ResourceGroup.attributes).joinedload(ResourceAttr.attr))
            .one()
        )
        ...
        raise ResourceNotFoundError(
            "ResourceGroup %s not found in network %s" % (group_name, network_id)
        )
...
def get_network_by_name(project_id, network_name, **kwargs):
    ...
    try:
        res = (
            db.DBSession.query(Network.id)
            .filter(
                func.lower(Network.name).like(network_name.lower()),
...
def network_exists(project_id, network_name, **kwargs):
    ...
    """
    try:
        db.DBSession.query(Network.id).filter(
            func.lower(Network.name).like(network_name.lower()),
            Network.project_id == project_id,
        ).one()
        return "Y"
    ...
    if network.nodes is not None and update_nodes is True:
        ...
        for node in network.nodes:
            ...
            if node.id is not None and node.id > 0:
                ...
                n = net_i.add_node(
                    node.name, node.description, node.get_layout(), node.x, node.y
                )
    ...
    if network.links is not None and update_links is True:
        ...
        for link in network.links:
            ...
            if link.id is None or link.id < 0:
                ...
                l = net_i.add_link(
                    link.name, link.description, link.get_layout(), node_1, node_2
                )
            ...
            if s.id is not None:
                if s.id > 0:
                    try:
                        scen_i = (
                            db.DBSession.query(Scenario)
                            .filter(Scenario.id == s.id)
                            .one()
                        )
                        if scen_i.locked == "Y":
                            errors.append(
                                "Scenario %s was not updated as it is locked" % (s.id)
                            )
                            continue
...
def update_resource_layout(resource_type, resource_id, key, value, **kwargs):
    log.info(
        "Updating %s %s's layout with {%s:%s}", resource_type, resource_id, key, value
    )
...
def add_node(network_id, node, **kwargs):
    ...
    if node.types is not None and len(node.types) > 0:
        ...
        for typesummary in node.types:
            ra, rt, rs = template.set_resource_type(
                new_node, typesummary.id, network_id=network_id, **kwargs
            )
            if rt is not None:
        ...
        if len(res_attrs) > 0:
            ...
            new_res_attrs = (
                db.DBSession.query(ResourceAttr)
...
def update_node(node, flush=True, **kwargs):
    ...
    node_i.description = (
        node.description if node.description is not None else node_i.description
    )
...
def _unique_data_qry(count=1):
    ...
    subqry = (
        db.DBSession.query(
            rs.dataset_id, func.count(rs.dataset_id).label("dataset_count")
        )
    ...
    unique_data = (
        db.DBSession.query(rs)
        .join(subqry, and_(rs.dataset_id == subqry.c.dataset_id))
        .filter(rs.resource_attr_id == ResourceAttr.id)
    )
    return unique_data
...
def _purge_datasets_unique_to_resource(ref_key, ref_id):
    ...
    count_qry = (
        db.DBSession.query(
            ResourceScenario.dataset_id, func.count(ResourceScenario.dataset_id)
        )
        .group_by(ResourceScenario.dataset_id)
    ...
    for dataset_id, count in count_rs:
        ...
        if full_dataset_count == count:
            ...
            log.info(
                "Deleting %s dataset %s (%s)",
                ref_key,
                dataset_to_delete.name,
                dataset_to_delete.id,
            )
...
def delete_node(node_id, purge_data, **kwargs):
    ...
    group_items = (
        db.DBSession.query(ResourceGroupItem)
        .filter(ResourceGroupItem.node_id == node_id)
        .all()
    )
...
def add_link(network_id, link, **kwargs):
    ...
    if link.types is not None and len(link.types) > 0:
        ...
        for typesummary in link.types:
            ra, rt, rs = template.set_resource_type(
                link_i, typesummary.id, network_id=network_id, **kwargs
            )
            res_types.append(rt)
            res_attrs.extend(ra)
...
def delete_link(link_id, purge_data, **kwargs):
    ...
    group_items = (
        db.DBSession.query(ResourceGroupItem)
        .filter(ResourceGroupItem.link_id == link_id)
        .all()
    )
    for gi in group_items:
        db.DBSession.delete(gi)
...
def add_group(network_id, group, **kwargs):
    ...
    if group.types is not None and len(group.types) > 0:
        ...
        for typesummary in group.types:
            ra, rt, rs = template.set_resource_type(
                res_grp_i, typesummary.id, network_id=network_id, **kwargs
            )
            res_types.append(rt)
            res_attrs.extend(ra)
            res_scenarios.update(rs)  # rs is a dict
...
def update_group(group, **kwargs):
    ...
    try:
        group_i = (
            db.DBSession.query(ResourceGroup).filter(ResourceGroup.id == group.id).one()
        )
    except NoResultFound:
    ...
    group_i.description = (
        group.description if group.description else group_i.description
    )
...
def set_group_status(group_id, status, **kwargs):
    ...
    try:
        group_i = (
            db.DBSession.query(ResourceGroup).filter(ResourceGroup.id == group_id).one()
        )
    except NoResultFound:
        raise ResourceNotFoundError("ResourceGroup %s not found" % (group_id))
...
def get_resources_of_type(network_id, type_id, **kwargs):
    ...
    nodes_with_type = (
        db.DBSession.query(Node)
        .join(ResourceType)
        .filter(Node.network_id == network_id, ResourceType.type_id == type_id)
        .all()
    )
    ...
    links_with_type = (
        db.DBSession.query(Link)
        .join(ResourceType)
        .filter(Link.network_id == network_id, ResourceType.type_id == type_id)
        .all()
    )
    ...
    groups_with_type = (
        db.DBSession.query(ResourceGroup)
        .join(ResourceType)
        .filter(ResourceGroup.network_id == network_id, ResourceType.type_id == type_id)
        .all()
    )
...
def get_all_node_data(
    network_id, scenario_id, node_ids=None, include_metadata=False, **kwargs
):
    resource_scenarios = get_attributes_for_resource(
        network_id,
        scenario_id,
        "NODE",
        ref_ids=node_ids,
...
def get_all_link_data(
    network_id, scenario_id, link_ids=None, include_metadata=False, **kwargs
):
    resource_scenarios = get_attributes_for_resource(
        network_id,
        scenario_id,
...
def get_all_group_data(
    network_id, scenario_id, group_ids=None, include_metadata=False, **kwargs
):
    resource_scenarios = get_attributes_for_resource(
        network_id,
        scenario_id,
...
def get_attributes_for_resource(
    network_id, scenario_id, ref_key, ref_ids=None, include_metadata=False, **kwargs
):
    ...
    try:
        db.DBSession.query(Scenario).filter(
            Scenario.id == scenario_id, Scenario.network_id == network_id
        ).one()
    except NoResultFound:
...
def get_all_resource_attributes_in_network(
    attr_id, network_id, include_resources=True, **kwargs
):
    """
    ...
    for ra in resourceattrs:
        ra_j = JSONObject(
            ra,
            extras={
                "node": JSONObject(ra.node) if ra.node else None,
                "link": JSONObject(ra.link) if ra.link else None,
                "resourcegroup": (
                    JSONObject(ra.resourcegroup) if ra.resourcegroup else None
                ),
        ...
        if ra.hidden == "Y":
            try:
                d = (
                    ...
                    .filter(Dataset.id == ra.dataset_id)
...
def clone_network(
    ...
    **kwargs,
    ...
    if project_id is None and new_project == True:
        ...
        ex_proj = (
            db.DBSession.query(Project).filter(Project.id == ex_net.project_id).one()
        )
        ...
        ex_project = (
            db.DBSession.query(Project)
            .filter(Project.name == project_name, Project.created_by == user_id)
            .all()
        )
        # If it exists, use it.
        if len(ex_project) > 0:
    ...
    ex_network = (
        db.DBSession.query(Network)
        .filter(
            Network.project_id == project_id, Network.name.like(f"{new_network_name}%")
        )
        .all()
    )
    ...
    newnet.description = (
        ex_net.description
        if new_network_description is None
        else new_network_description
    )
    ...
    group_id_map = _clone_groups(
        network_id, newnetworkid, node_id_map, link_id_map, user_id
    )

    log.info("Cloning Resource Attributes")
    ra_id_map = _clone_resourceattrs(
        network_id,
        newnetworkid,
        node_id_map,
        link_id_map,
    ...
    _clone_resourcetypes(
        network_id, newnetworkid, node_id_map, link_id_map, group_id_map
    )

    log.info("Cloning Scenarios")
    scenario_id_map = _clone_scenarios(
        network_id,
        newnetworkid,
        ra_id_map,
        node_id_map,
...
def clone_node(
    node_id, include_outputs=False, name=None, new_x=None, new_y=None, **kwargs
):
    """
    ...
    node_net = (
        db.DBSession.query(Network)
        .filter(Network.id == Node.network_id, Node.id == node_id)
        .one()
    )
...
def clone_nodes(
    ...
    **kwargs,
    ...
    node_net = (
        db.DBSession.query(Network)
        .filter(Network.id == Node.network_id, Node.id == node_ids[0])
        .one()
    )
    ...
    if None not in (new_x_list, new_y_list) and len(new_x_list) != len(new_y_list):
        raise HydraError(
            "Unable to clone nodes. The list of x coordinates must match the length of y coordinates"
        )

    if names is not None and len(names) != len(new_x_list):
        raise HydraError(
            "Unable to clone nodes. A name must be specified for each cloned node, or the names argument must be None. "
        )

    node_net.check_write_permission(user_id)
    cloned_ids = []
    for i, node_id in enumerate(node_ids):
        cloned_id = _clone_node(
            node_id,
            node_net,
            user_id,
            include_outputs=include_outputs,
...
def _clone_node(
    node_id, node_net, user_id, include_outputs=False, name=None, new_x=None, new_y=None
):
    ...
    if name is not None:
        node_with_same_name = (
            db.DBSession.query(Node)
            .filter(Node.network_id == node_net.id, Node.name == name)
            .all()
        )
    ...
    if new_x is not None:
        try:
            ...
            raise HydraError(
                f"Unable to clone node {name}. Coordinate {new_x} must be numeric."
            )
    ...
    if new_y is not None:
        try:
            ...
            raise HydraError(
                f"Unable to clone node {name}. Coordinate {new_y} must be numeric."
            )
    ...
    node_ras = (
        db.DBSession.query(ResourceAttr)
        .filter(and_(ResourceAttr.node_id == node_id))
        .all()
    )
    new_ras = []
    ra_id_map = {}
    ...
    new_node_ras = (
        db.DBSession.query(ResourceAttr)
        .filter(ResourceAttr.node_id == newnode.id)
        .all()
    )
    ...
    log.info("Cloning Resource Types")
    node_rts = (
        db.DBSession.query(ResourceType)
        .filter(and_(ResourceType.node_id == node_id))
        .all()
    )
    new_resourcetypes = []
    for rt in node_rts:
        new_resourcetypes.append(
            dict(
                ref_key=rt.ref_key,
                node_id=newnode.id,
                type_id=rt.type_id,
    ...
    node_rules = db.DBSession.query(Rule).join(Node).filter(Node.id == node_id).all()
...
def _clone_rules(
    ...
    node_rules = (
        ...
        .join(Node)
        .filter(Node.network_id == old_network_id)
        .all()
    )
    for node_rule in node_rules:
        rules.clone_rule(
    ...
    link_rules = (
        db.DBSession.query(Rule)
        .join(Link)
        .filter(Link.network_id == old_network_id)
        .all()
    )
    ...
    group_rules = (
        db.DBSession.query(Rule)
        .join(ResourceGroup)
        .filter(ResourceGroup.network_id == old_network_id)
        .all()
    )
...
def _clone_groups(old_network_id, new_network_id, node_id_map, link_id_map, user_id):
    ...
    groups = (
        db.DBSession.query(ResourceGroup)
        .filter(ResourceGroup.network_id == new_network_id)
        .all()
    )
    for g in groups:
...
def _clone_attributes(
    network_id, newnetworkid, exnet_project_id, newnet_project_id, user_id
):
    """
    ...
    network_scoped_attrs = attributes.get_attributes(
    ...
    if exnet_project_id != newnet_project_id:
        ...
        new_attributes = []
        exnet_project_scoped_attrs = attributes.get_attributes(
            project_id=exnet_project_id, user_id=user_id
        )
...
def _clone_resourceattrs(
    ...
    new_scoped_attr_lookup = _clone_attributes(
        network_id, newnetworkid, exnet_project_id, newnet_project_id, user_id
    )

    log.info("Cloning Network Attributes")
    network_ras = db.DBSession.query(ResourceAttr).filter(
    ...
    node_ras = (
        db.DBSession.query(ResourceAttr)
        .filter(and_(ResourceAttr.node_id == Node.id, Node.network_id == network_id))
        .all()
    )
    for ra in node_ras:
        new_attr = new_scoped_attr_lookup.get(ra.attr_id)
    ...
    link_ras = (
        db.DBSession.query(ResourceAttr)
        .filter(and_(ResourceAttr.link_id == Link.id, Link.network_id == network_id))
        .all()
    )
    for ra in link_ras:
        new_attr = new_scoped_attr_lookup.get(ra.attr_id)
    ...
    new_network_ras = (
        db.DBSession.query(ResourceAttr)
        .filter(ResourceAttr.network_id == newnetworkid)
        .all()
    )
    for ra in new_network_ras:
        id_map[
            old_ra_name_map[
                (ra.network_id, ra.node_id, ra.link_id, ra.group_id, ra.attr_id)
            ]
        ] = ra.id

    new_node_ras = (
        db.DBSession.query(ResourceAttr)
        .filter(and_(ResourceAttr.node_id == Node.id, Node.network_id == newnetworkid))
        .all()
    )
    for ra in new_node_ras:
        id_map[
    ...
    new_link_ras = (
        db.DBSession.query(ResourceAttr)
        .filter(and_(ResourceAttr.link_id == Link.id, Link.network_id == newnetworkid))
        .all()
    )
    ...
    new_group_ras = (
        ...
        .filter(
            and_(
                ResourceAttr.group_id == ResourceGroup.id,
                ResourceGroup.network_id == newnetworkid,
            )
        )
        .all()
    )
...
def _clone_resourcetypes(
    ...
    node_rts = (
        db.DBSession.query(ResourceType)
        .filter(and_(ResourceType.node_id == Node.id, Node.network_id == network_id))
        .all()
    )
    for rt in node_rts:
        new_rts.append(
    ...
    link_rts = (
        db.DBSession.query(ResourceType)
        .filter(and_(ResourceType.link_id == Link.id, Link.network_id == network_id))
        .all()
    )
    for rt in link_rts:
        new_rts.append(
...
def _clone_scenarios(
    ...
    scenarios = (
        db.DBSession.query(Scenario).filter(Scenario.network_id == network_id).all()
    )
    ...
    for scenario in scenarios:
        ...
        if scenario.status == "A":
            new_scenario_id = _clone_scenario(
                scenario,
                newnetworkid,
                ra_id_map,
                node_id_map,
...
def _clone_scenario(
    ...
    old_rgis = (
        db.DBSession.query(ResourceGroupItem)
        .filter(ResourceGroupItem.scenario_id == old_scenario.id)
        .all()
    )
