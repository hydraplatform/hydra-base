...
def check_dimension(typeattr, unit_id=None):
    ...
    if unit_id is not None and dimension_id is None:
        ...
        dimension = units.get_dimension(
            unit_dimension_id, do_accept_dimension_id_none=True
        )
        raise HydraError(
            f"Unit {unit_id} ({unit.abbreviation}) has dimension_id"
        ...
        unit_dimension_id = units.get_dimension_by_unit_id(unit_id).id
        unit = units.get_unit(unit_id)
        dimension1 = units.get_dimension(
            unit_dimension_id, do_accept_dimension_id_none=True
        )
        dimension2 = units.get_dimension(
            unit_dimension_id, do_accept_dimension_id_none=True
        )
...
def get_attr_by_name_and_dimension(name, dimension_id):
    ...
    attr = (
        ...
        .filter(Attr.name == name, Attr.dimension_id == dimension_id)
        .first()
    )
    ...
    if attr is None:
        ...
        log.debug(
            "Attribute not found, creating new attribute: name:%s, dimen:%s",
            attr.name,
            attr.dimension_id,
        )
