...
def get_network_template(network_id, reference_type_id):
    ...
    ref_type = (
        ...
        .filter(TemplateType.id == reference_type_id)
        .one()
    )
    ...
    for nt in network_types:
        ...
        network_template_parent_id = (
            db.DBSession.query(Template)
            .filter(Template.id == network_tt.template_id)
            .one()
            .parent_id
        )
...
def _get_type(type_id):
    ...
    template_i = (
        db.DBSession.query(Template).filter(Template.id == type_i.template_id).one()
    )
        ...
        for template_type in network_template_i.get_types():
            if template_type.resource_type == "NETWORK":
                ...
                assign_type_to_resource(
                    network_type_id, "NETWORK", network_id, **kwargs
                )
                break
        else:
            raise NoResultFound(
                "A template type with resource type NETWORK "
    ...
    resource_types = (
        ...
        .filter(
            and_(
                ...
                ResourceType.type_id.in_(type_ids),
            )
        )
        .all()
    )
...
def get_matching_resource_types(resource_type, resource_id, **kwargs):
    ...
    if resource_type == "NETWORK":
        ...
        resource_i = (
            ...
            .filter(ResourceGroup.id == resource_id)
            .one()
        )
...
def assign_types_to_resources(resource_types, template_id=None, **kwargs):
    ...
    if len(res_attrs) > 0:
        ...
        new_ras = (
            ...
            .filter(
                and_(
                    ResourceAttr.id >= last_row_id,
                    ResourceAttr.id < (last_row_id + len(res_attrs)),
                )
            )
            .all()
        )
...
def check_type_compatibility(type_1_id, type_2_id, **kwargs):
    ...
    for ta in type_2.typeattrs:
        ...
        if type_1_unit_id is None and type_2_unit_id is not None:
            errors.append(
                "Type %(type_name)s in template %(template_1_name)s"
            ...
            errors.append(
                ...
                " template %(template_2_name)s stores it in %(type_2_unit_id)s"
                % fmt_dict
            )
        return errors
...
def _get_links(link_ids):
    ...
    while lower < limit:
        ...
        rs = (
            ...
            .filter(Link.id.in_(link_ids[lower:upper]))
            .all()
        )
...
def _get_nodes(node_ids):
    ...
    while lower < limit:
        ...
        rs = (
            ...
            .filter(Node.id.in_(node_ids[lower:upper]))
            .all()
        )
...
def _get_groups(group_ids):
    ...
    while lower < limit:
        ...
        rs = (
            ...
            .filter(ResourceGroup.id.in_(group_ids[lower:upper]))
            .all()
        )
...
def assign_type_to_resource(type_id, resource_type, resource_id, **kwargs):
    ...
    if resource_type != type_i.resource_type:
        raise HydraError(
            "Cannot assign a %s type to a %s" % (type_i.resource_type, resource_type)
        )
...
def set_resource_type(resource, type_id, types={}, **kwargs):
    ...
    for typeattr in type_i.typeattrs:
        type_attrs[typeattr.attr_id] = {
            ...
            "default_dataset_id": (
                typeattr.default_dataset.id if typeattr.default_dataset else None
            ),
        }
    ...
    for rt in resource.types:
        ...
        if len(errors) > 0:
            raise HydraError(
                "Cannot apply type %s to resource %s as it "
                "conflicts with type %s. Errors are: %s"
                % (
                    type_i.name,
                    resource.get_name(),
                    rt.get_templatetype().name,
                    ",".join(errors),
                )
            )
    else:
...
def remove_type_from_resource(type_id, resource_type, resource_id, **kwargs):
    ...
    resourcetype = (
        ...
        .filter(
            ...
            ResourceType.group_id == group_id,
        )
        .one()
    )
...
def validate_attr(resource_attr_id, scenario_id, template_id=None, **kwargs):
    ...
    rs = (
        ...
        .options(joinedload(ResourceScenario.dataset))
        .one()
    )
...
def validate_resourcescenario(resourcescenario, template_id=None, **kwargs):
    ...
    if template_id is not None:
        if template_id not in [r.get_templatetype().template_id for r in res.types]:
            raise HydraError(
                "Template %s is not used for resource attribute %s in scenario %s"
                % (
                    template_id,
                    resourcescenario.resourceattr.attr.name,
...
def validate_network(network_id, template_id, scenario_id=None, **kwargs):
    ...
    if scenario_id is not None:
        scenario = (
            db.DBSession.query(Scenario).filter(Scenario.id == scenario_id).first()
        )
    ...
    template = db.DBSession.query(Template).filter(Template.id == template_id).first()
...
def validate_resource(resource, tmpl_types, resource_scenarios=[], **kwargs):
    ...
    if len(resource_scenarios) > 0:
        for ra_id in resource_attr_ids:
            ...
            rs_dimension_id = units.get_dimension_by_unit_id(
                rs_unit_id, do_accept_unit_id_none=True
            ).id

            attr = (
                db.DBSession.query(Attr)
                .filter(Attr.id == rs.resourceattr.attr_id)
                .one()
            )
            ...
            if rs_dimension_id != type_dimension_id:
                errors.append(
                    "Dimension mismatch on %s %s, attribute %s: "
