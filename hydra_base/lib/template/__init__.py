...
from hydra_base.lib.template.xml import (
    get_template_as_xml,
    import_template_xml,
    get_network_as_xml_template,
)

from hydra_base.lib.template.resource import (
    get_types_by_attr,
...
def parse_json_typeattr(
    type_i, typeattr_j, attribute_j, default_dataset_j, user_id=None
):
    dimension_i = None
    ...
    if default_dataset_j is not None:
        ...
        name = (
            default.name
            if default.name not in (None, "")
            else "%s Default" % attr_i.name
        )

        dataset_i = add_dataset(data_type, val, unit_id, name=name, user_id=user_id)
    ...
    if typeattr_j.restriction is not None or typeattr_j.data_restriction is not None:
        restriction = (
            typeattr_j.restriction
            if typeattr_j.restriction is not None
            else typeattr_j.data_restriction
        )
...
def get_template_as_dict(template_id, **kwargs):
    ...
    for tmpltype_j in template_j.templatetypes:
        ...
        for typeattr_j in tmpltype_j.typeattrs:
            ...
            if (
                hasattr(typeattr_j, "default_dataset")
                and typeattr_j.default_dataset is not None
            ):
...
def import_template_json(template_json_string, allow_update=True, **kwargs):
    ...
    return import_template_dict(
        template_dict, allow_update=allow_update, user_id=user_id
    )
...
def import_template_dict(template_dict, allow_update=True, **kwargs):
    ...
    if file_attributes is None or len(template_j) == 0:
        raise HydraError(
            "Invalid template. The template must have the following structure: "
    ...
    for type_j in types_j:
        ...
        if type_j.resource_type is None:
            raise HydraError(
                "No resource type specified." " 'NODE', 'LINK', 'GROUP' or 'NETWORK'"
            )
        ...
        for typeattr_j in type_j.typeattrs:
            ...
            if typeattr_j.default_dataset is not None:
                ...
                default_dataset_j = default_datasets_j[
                    int(typeattr_j.default_dataset_id)
                ]

            parse_json_typeattr(
                type_i, typeattr_j, attr_j, default_dataset_j, user_id=user_id
            )
...
def add_child_template(parent_id, name, description=None, **kwargs):
    ...
    """
    parent_template = (
        db.DBSession.query(Template).filter(Template.id == parent_id).one()
    )
    ...
    if template.types is not None or template.templatetypes is not None:
        ...
        for templatetype in types:
            ...
            if templatetype.id is not None and templatetype.template_id != tmpl.id:
                log.debug(
                    "Type %s is a part of a parent template. Ignoring.", templatetype.id
                )
...
def get_templates(load_all=True, include_inactive=False, **kwargs):
    ...
    """
    templates_i = (
        db.DBSession.query(Template).options(joinedload(Template.templatetypes)).all()
    )
    if load_all is True:
        full_templates = []
...
def remove_attr_from_type(type_id, attr_id, **kwargs):
    ...
    typeattr_i = (
        db.DBSession.query(TypeAttr)
        .filter(TypeAttr.type_id == type_id, TypeAttr.attr_id == attr_id)
        .one()
    )
...
def get_template_by_name(name, **kwargs):
    ...
    """
    try:
        tmpl_i = db.DBSession.query(Template).filter(Template.name == name).one()
...
def add_child_templatetype(parent_id, child_template_id, **kwargs):
    ...
    existing_child = (
        ...
        .filter(
            ...
            TemplateType.template_id == child_template_id,
        )
        .first()
    )
    ...
    parent_type = (
        db.DBSession.query(TemplateType).filter(TemplateType.id == parent_id).one()
    )
...
def add_child_typeattr(parent_id, child_template_id, **kwargs):
    ...
    existing_child_typeattr = (
        db.DBSession.query(TypeAttr)
        .join(TemplateType)
        .filter(TypeAttr.parent_id == parent_id)
        .filter(TemplateType.template_id == child_template_id)
        .first()
    )
    ...
    parent_typeattr = (
        db.DBSession.query(TypeAttr).filter(TypeAttr.id == parent_id).one()
    )
...
def update_templatetype(templatetype, auto_delete=False, **kwargs):
    ...
    tmpltype_i = (
        db.DBSession.query(TemplateType)
        .filter(TemplateType.id == templatetype.id)
        .one()
    )
    ...
    ***WARNING***
    ...
    if existing_ta is None:
        ...
        check_existing_ta = (
            db.DBSession.query(TypeAttr)
            .filter(
                TypeAttr.attr_id == typeattr.attr_id,
    ...
    if (
        hasattr(typeattr, "default_dataset_id")
        and typeattr.default_dataset_id is not None
    ):
    ...
    if typeattr.unit_id is None or typeattr.unit_id == "":
        ...
        if typeattr.attr_id is not None and typeattr.attr_id > 0:
            ...
            if (
                attr is not None
                and attr.dimension_id is not None
                and attr.dimension_id != dimension.id
                or attr is not None
                and attr.dimension_id is None
            ):
...
def _set_cols(source, target, reference=None):
    ...
    for colname in source:
        ...
        if (
            hasattr(reference, "_protected_columns")
...
def delete_templatetype(
    ...
    **kwargs,
    ...
    try:
        tmpltype_i = (
            db.DBSession.query(TemplateType).filter(TemplateType.id == type_id).one()
        )
    except NoResultFound:
    ...
    if template_i is None:
        template_i = (
            db.DBSession.query(Template)
            .filter(Template.id == tmpltype_i.template_id)
            .one()
        )
...
def get_templatetype(type_id, include_parent_data=True, **kwargs):
    ...
    templatetype = (
        ...
        .options(noload(TemplateType.typeattrs))
        .one()
    )
    ...
    template = (
        db.DBSession.query(Template)
        .filter(Template.id == templatetype.template_id)
        .one()
    )
...
def get_templatetype_by_name(template_id, type_name, **kwargs):
    ...
    try:
        templatetype = (
            db.DBSession.query(TemplateType)
            .filter(
                TemplateType.template_id == template_id, TemplateType.name == type_name
            )
            .one()
        )
    except NoResultFound:
...
def update_typeattr(typeattr, **kwargs):
    ...
    if typeattr.id is not None:
        existing_ta = (
            db.DBSession.query(TypeAttr).filter(TypeAttr.id == typeattr.id).first()
        )
    else:
        existing_ta = (
            db.DBSession.query(TypeAttr)
