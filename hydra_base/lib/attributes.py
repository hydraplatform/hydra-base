    ...
    try:
        if ref_key == "NODE":
            ...
            return (
                ...
                .filter(ResourceGroup.id == ref_id)
                .one()
            )
        ...
        raise ResourceNotFoundError(
            "Resource %s with ID %s not found" % (ref_key, ref_id)
        )
...
def get_attribute_by_id(attr_id, **kwargs):
    ...
    try:
        ...
        raise ResourceNotFoundError(
            "Attribute (attribute id=%s) does not exist" % (attr_id)
        )
...
def get_template_attributes(template_id, **kwargs):
    ...
    try:
        attrs_i = (
            db.DBSession.query(Attr)
            .filter(TemplateType.template_id == template_id)
            .filter(TypeAttr.type_id == TemplateType.id)
            .filter(Attr.id == TypeAttr.id)
            .all()
        )
...
def get_attribute_by_name_and_dimension(
    name, dimension_id=None, network_id=None, project_id=None, **kwargs
):
    """
    ...
    log.info(
        "Retrieving all attributes with name %s and dimension %s (network_id=%s) (project_id=%s)",
        name,
        dimension_id,
        network_id,
        project_id,
...
def search_attributes(name, network_id=None, project_id=None, **kwargs):
    ...
    try:
        ...
        if project_id is not None:
            ...
            proj_attrs_i = proj_i.get_scoped_attributes(
                name_match=name, include_hierarchy=True
            )
        ...
        if network_id is not None:
            ...
            net_attrs_i = net_i.get_scoped_attributes(
                name_match=name, include_hierarchy=include_network_hierarchy
            )
        ...
        for a_j in attrs_dict.values():
            if a_j.dimension_id is not None:
                dimension_i = (
                    db.DBSession.query(Dimension)
                    .filter(Dimension.id == a_j.dimension_id)
                    .one()
                )
...
def _add_attribute(attr, user_id, flush=True, do_reassign=False):
    ...
    _check_can_add_attribute(
        attr.name, attr.dimension_id, attr.project_id, attr.network_id
    )

    if attr.network_id is not None and attr.project_id is not None:
        raise HydraError(
            f"Unable to add attrubute {attr.name}. "
...
def _reassign_scoped_attributes(attr_id, user_id):
    ...
    matching_attrs = get_attributes_by_name_and_dimension(
        attr_i.name, attr_i.dimension_id
    )
    ...
    if attr_i.project_id is not None:
        ...
        attr_proj = (
            db.DBSession.query(Project).filter(Project.id == attr_i.project_id).one()
        )
        child_projects = attr_proj.get_child_projects(
            user_id=user_id, levels=max_levels
        )
        project_scope = {p["id"] for p in child_projects} | {attr_i.project_id}
        network_scoped_resource_attrs_qry = scoped_resource_attrs_qry.join(
            Network
        ).filter(Attr.network_id == Network.id, Network.project_id.in_(project_scope))

        project_scoped_resource_attrs_qry = scoped_resource_attrs_qry.join(
            Project
        ).filter(Attr.project_id.in_(project_scope))

        scoped_resource_attrs = (
            network_scoped_resource_attrs_qry.all()
            + project_scoped_resource_attrs_qry.all()
        )

    if len(scoped_resource_attrs) > 0:
        log.info(
            f"{len(scoped_resource_attrs)} scoped attributes found with same name & dimension. Reassigning."
        )
    ...
    for matching_attr in matching_attrs:
        if matching_attr.id != attr_id and (
            matching_attr.project_id in project_scope
            or matching_attr.network_id in rescoped_from_networks
        ):
...
def add_attribute(attr, check_existing=True, **kwargs):
    ...
    try:
        ...
        if attr.network_id is not None and attr.project_id is None:
            ...
            attr_qry = attr_qry.filter(
                or_(
                    Attr.network_id == attr.network_id,
...
def update_attribute(attr, **kwargs):
    ...
    if existing_attr_i is not None:
        ...
        raise HydraError(
            f"Cannot update attribute. An attribute with name {attr.name}"
...
def _get_projects_referenced_by_network_id(network_id, **kwargs):
    ...
    project_id_rs = (
        db.DBSession.query(Network.project_id).filter(Network.id == network_id).one()
    )
...
def add_attributes(attrs, **kwargs):
    ...
    global_attrs = (
        db.DBSession.query(Attr)
        .filter(and_(Attr.network_id == None, Attr.project_id == None))
        .all()
    )
    ...
    network_ids = list(
        set(
            [
                a.network_id
                for a in filter(
                    lambda x: x.network_id is not None and x.project_id is None, attrs
                )
            ]
        )
    )
    ...
    direct_project_ids = list(
        set([a.project_id for a in filter(lambda x: x.project_id is not None, attrs)])
    )
    ...
    if len(direct_project_ids) > 0:
        project_ids = _get_projects_referenced_by_project_id(
            direct_project_ids[0], **kwargs
        )
    ...
    for project_id in project_ids + network_project_ids:
        ...
        project_attrs = (
            db.DBSession.query(Attr).filter(Attr.project_id == project_id).all()
        )
        for project_attr in project_attrs:
            project_attr_dict[(project_attr.name, project_attr.dimension)] = (
                project_attr
            )
    ...
    if len(network_ids) > 0:
        network_attrs = (
            db.DBSession.query(Attr).filter(Attr.network_id.in_(network_ids)).all()
        )
    ...
    **kwargs,
    ...
    if (network_id is None and project_id is None) or include_global is True:
        ...
        attrs = base_qry.filter(
            and_(Attr.network_id == None, Attr.project_id == None)
        ).all()
    ...
    if project_id is not None:
        ...
        project_scoped_attributes = project.get_scoped_attributes(
        ...
        if network_id is None and include_network_attributes is True:
            ...
            network_attributes = base_qry.filter(
                Attr.network_id.in_([n.id for n in nets])
            ).all()
    ...
    if network_id is not None:
        ...
        if project_id is not None:
            include_network_hierarchy = False
        network_scoped_attributes = net.get_scoped_attributes(
            include_hierarchy=include_network_hierarchy
        )
...
def get_attributes_by_name_and_dimension(name, dimension_id=None, **kwargs):
    ...
    """
    log.info(
        "Retrieving all attributes with name %s and dimension %s", name, dimension_id
    )
    attr_qry = db.DBSession.query(Attr).filter(
        and_(
            func.lower(Attr.name) == name.strip().lower(),
...
def update_resource_attribute(resource_attr_id, is_var, **kwargs):
    ...
    try:
        ra = (
            db.DBSession.query(ResourceAttr)
            .filter(ResourceAttr.id == resource_attr_id)
            .one()
        )
    except NoResultFound:
        raise ResourceNotFoundError(
            "Resource Attribute %s not found" % (resource_attr_id)
        )
...
def add_resource_attribute(
    resource_type, resource_id, attr_id, is_var, error_on_duplicate=True, **kwargs
):
    """
    ...
    resourceattr_qry = db.DBSession.query(ResourceAttr).filter(
        ResourceAttr.ref_key == resource_type
    )

    if resource_type == "NETWORK":
        resourceattr_qry = resourceattr_qry.filter(
            ResourceAttr.network_id == resource_id
        )
        ...
        resourceattr_qry = resourceattr_qry.filter(
            ResourceAttr.project_id == resource_id
        )
    else:
...
def get_all_network_resourceattributes(
    network_id, template_id=None, return_orm=False, **kwargs
):
    """
    ...
    resource_attr_qry = (
        ...
        .outerjoin(ResourceGroup, ResourceGroup.id == ResourceAttr.group_id)
        .filter(
            or_(
                and_(
                    ResourceAttr.network_id != None,
    ...
    if template_id is not None:
        ...
        rs = (
            ...
            .join(TemplateType, TemplateType.id == TypeAttr.type_id)
            .filter(TemplateType.template_id == template_id)
            .all()
        )
...
def get_all_network_attributes(network_id, template_id=None, **kwargs):
    ...
    network_attr_qry = (
        db.DBSession.query(Attr, ResourceAttr.attr_is_var)
        .join(ResourceAttr, ResourceAttr.attr_id == Attr.id)
        .join(Network, Network.id == ResourceAttr.network_id)
        .filter(
            and_(ResourceAttr.network_id != None, ResourceAttr.network_id == network_id)
        )
    )
    ...
    node_attr_qry = (
        ...
        .filter(
            and_(
                ResourceAttr.node_id is not None,
                ResourceAttr.node_id == Node.id,
                Node.network_id == network_id,
            )
        )
    ...
    link_attr_qry = (
        ...
        .filter(
            and_(
                ResourceAttr.link_id is not None,
                ResourceAttr.link_id == Link.id,
                Link.network_id == network_id,
            )
        )
    ...
    group_attr_qry = (
        ...
        .filter(
            and_(
                ResourceAttr.group_id is not None,
                ResourceAttr.group_id == ResourceGroup.id,
                ResourceGroup.network_id == network_id,
            )
        )
    ...
    if template_id is not None:
        log.info(
            "Filtering out only attributes which appear in template %s", template_id
        )
        attr_ids = []
        rs = (
            db.DBSession.query(TypeAttr)
        ...
        log.info(
            "Filtered out %s attributes", len(resource_attrs) - len(filtered_results)
        )
...
def check_attr_dimension(attr_id, **kwargs):
    ...
    for d in datasets:
        if (
            attr_i.dimension_id is None
            and d.unit is not None
            or attr_i.dimension_id is not None
            and d.unit is None
            or units.get_dimension_by_unit_id(d.unit_id) != attr_i.dimension_id
        ):
    ...
    if len(bad_datasets) > 0:
        raise HydraError(
            "Datasets %s have a different dimension_id to attribute %s"
...
def get_resource_attribute(resource_attr_id, **kwargs):
    ...
    if resource_attr is None:
        raise ResourceNotFoundError(
            f"Resource attribute {resource_attr_id} does not exist"
        )
...
def delete_attribute_mapping(resource_attr_a, resource_attr_b, **kwargs):
    ...
    log.info(
        "Trying to delete attribute map. %s -> %s", resource_attr_a, resource_attr_b
    )
    mapping = (
        db.DBSession.query(rm)
        .filter(
            rm.resource_attr_id_a == resource_attr_a,
            rm.resource_attr_id_b == resource_attr_b,
        )
        .first()
    )
...
def delete_mappings_in_network(network_id, network_2_id=None, **kwargs):
    ...
    qry = db.DBSession.query(ResourceAttrMap).filter(
        or_(
            ResourceAttrMap.network_a_id == network_id,
    ...
    if network_2_id is not None:
        qry = qry.filter(
            or_(
                ResourceAttrMap.network_a_id == network_2_id,
...
def check_attribute_mapping_exists(
    resource_attr_id_source, resource_attr_id_target, **kwargs
):
    """
    ...
    """
    qry = (
        db.DBSession.query(ResourceAttrMap)
        .filter(
...
def add_attribute_group(attributegroup, **kwargs):
    ...
    """
    log.info(
        "attributegroup.project_id %s", attributegroup.project_id
    ...
    try:
        ...
        group_i = (
            db.DBSession.query(AttrGroup)
            .filter(
                AttrGroup.name == attributegroup.name,
                AttrGroup.project_id == attributegroup.project_id,
            )
            .one()
        )
        log.info(
            "Group %s already exists in project %s",
            attributegroup.name,
            attributegroup.project_id,
        )
        ...
        log.info(
            "Attribute Group %s added to project %s",
            attributegroup.name,
            attributegroup.project_id,
        )
...
def update_attribute_group(attributegroup, **kwargs):
    ...
    try:
        ...
        group_i = (
            db.DBSession.query(AttrGroup)
            .filter(AttrGroup.id == attributegroup.id)
            .one()
        )
        ...
        raise HydraError(
            "No Attribute Group %s was found in project %s",
            attributegroup.id,
            attributegroup.project_id,
        )
...
def get_network_attributegroup_items(network_id, **kwargs):
    ...
    group_items_i = (
        db.DBSession.query(AttrGroupItem)
        .filter(AttrGroupItem.network_id == network_id)
        .all()
    )
...
def get_group_attributegroup_items(network_id, group_id, **kwargs):
    ...
    group_items_i = (
        db.DBSession.query(AttrGroupItem)
        .filter(
            AttrGroupItem.network_id == network_id, AttrGroupItem.group_id == group_id
        )
        .all()
    )
...
def get_attribute_item_groups(network_id, attr_id, **kwargs):
    ...
    group_items_i = (
        db.DBSession.query(AttrGroupItem)
        .filter(
            AttrGroupItem.network_id == network_id, AttrGroupItem.attr_id == attr_id
        )
        .all()
    )
...
def _get_attr_group(group_id):
    try:
        ...
        raise HydraError(
            "Error adding attribute group item: group %s not found" % (group_id)
        )
...
def add_attribute_group_items(attributegroupitems, **kwargs):
    ...
    ** attributegroupitems : a list of items, of the form:
    ...
    if not isinstance(attributegroupitems, list):
        raise HydraError(
            "Cannpt add attribute group items. Attributegroupitems must be a list"
        )
            ...
            for net_agi in network_agis:
                ...
                if agi_lookup.get(net_agi.network_id) is None:
                    ...
                    if agi_lookup[net_agi.network_id].get(net_agi.attr_id) is None:
                        agi_lookup[net_agi.network_id][net_agi.attr_id] = [
                            net_agi.group_id
                        ]
                    elif (
                        net_agi.group_id
                        not in agi_lookup[net_agi.network_id][net_agi.attr_id]
                    ):
                        agi_lookup[net_agi.network_id][net_agi.attr_id].append(
                            net_agi.group_id
                        )
        ...
        if agi_lookup[agi.network_id].get(agi.attr_id) is not None:
            for group_id in agi_lookup[agi.network_id][agi.attr_id]:
                ...
                if group.exclusive == "Y":
                    ...
                    raise HydraError(
                        "Attribute %s is already in Group %s for network %s. This group is exclusive, so attr %s cannot exist in another group."
                        % (agi.attr_id, group.id, agi.network_id, agi.attr_id)
                    )
            ...
            if group_lookup[agi.group_id].exclusive == "Y":
                if len(agi_lookup[agi.network_id][agi.attr_id]) > 0:
                    # The other group is exclusive, so this attr can't be added
                    raise HydraError(
                        "Cannot add attribute %s to group %s. This group is exclusive, but attr %s has been found in other groups (%s)"
                        % (
                            agi.attr_id,
                            agi.group_id,
                            agi.attr_id,
                            agi_lookup[agi.network_id][agi.attr_id],
                        )
                    )
    ...
    ** attributegroupitems : a list of items, of the form:
    ...
    for agi in attributegroupitems:
        ...
        agi_i = (
            db.DBSession.query(AttrGroupItem)
            .filter(
                AttrGroupItem.network_id == agi.network_id,
                AttrGroupItem.group_id == agi.group_id,
                AttrGroupItem.attr_id == agi.attr_id,
            )
            .first()
        )
...
def delete_all_duplicate_attributes(**kwargs):
    ...
    for dupe_list in duplicate_attributes:
        log.info(
            "Duplicate attributes found: name: %s, dimension: %s",
            dupe_list[0].name,
            dupe_list[0].dimension_id,
        )
...
def delete_duplicate_resourceattributes(network_id=None, **kwargs):
    ...
    if network_id is None:
        ...
        all_ras = get_all_network_resourceattributes(
            network_id, return_orm=True, **kwargs
        )
    ...
    for duplicate_ras in duplicate_ra_list:
        ...
        for duplicate_ra in duplicate_ras:
            if duplicate_ra.attr_id not in resource_typeattrs:
                ...
                ra_rs = (
                    ...
                    .filter(ResourceScenario.resource_attr_id == duplicate_ra.id)
                    .first()
                )
        ...
        if len(ras_to_delete) == len(duplicate_ras):
            for ra_to_delete in ras_to_delete:
                ra_rs = (
                    db.DBSession.query(ResourceScenario)
                    .filter(ResourceScenario.resource_attr_id == ra_to_delete.id)
                    .first()
                )
                if ra_rs is None:
                    log.info(
                        "A duplicate found for %s on %s %s. Deleting as it has no data.",
                    ...
                    log.info(
                        "A duplicate found for %s on %s %s. Not deleting as it has data.",
                        ra_to_delete.attr.name,
                        ra_to_delete.ref_key,
                        resource.name,
                    )
            ...
            if duplicate_ra.attr_id in resource_typeattrs:
                ...
                if ra_rs is None:
                    ...
                    if dupes_rs is not None:
                        log.info(
                            "Updating resource scenario with new reference to %s",
        ...
        for ra_to_delete in ras_to_delete:
            ...
            log.info(
                "Deleting resource attr %s (id: %s) from %s %s",
                duplicate_ra.attr.name,
                duplicate_ra.id,
...
def remap_attribute_reference(old_attr_id, new_attr_id, flush=False):
    ...
    ras_to_remap = (
        db.DBSession.query(ResourceAttr)
        .filter(ResourceAttr.attr_id == old_attr_id)
        .all()
    )
    ...
    for ra_to_remap in ras_to_remap:
        ...
        existing_ra = (
            db.DBSession.query(ResourceAttr)
            .filter(
                ResourceAttr.attr_id == new_attr_id,
                ...
                ResourceAttr.network_id == ra_to_remap.network_id,
            )
            .first()
        )
        ...
        if existing_ra is not None:
            ...
            old_ra_rs = (
                db.DBSession.query(ResourceScenario)
                .filter(ResourceScenario.resource_attr_id == ra_to_remap.id)
                .first()
            )
            new_ra_rs = (
                db.DBSession.query(ResourceScenario)
                .filter(ResourceScenario.resource_attr_id == existing_ra.id)
                .first()
            )
    ...
    tas_to_remap = (
        db.DBSession.query(TypeAttr).filter(TypeAttr.attr_id == old_attr_id).all()
    )
    ...
    for ta_to_remap in tas_to_remap:
        ...
        existing_ta = (
            db.DBSession.query(TypeAttr)
            .filter(
                TypeAttr.attr_id == new_attr_id, TypeAttr.type_id == ta_to_remap.type_id
            )
            .first()
        )
