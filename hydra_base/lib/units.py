...
def exists_dimension(dimension_name, **kwargs):
    ...
    try:
        dimension = (
            db.DBSession.query(Dimension).filter(Dimension.name == dimension_name).one()
        )
...
def convert_units(
    values,
    source_measure_or_unit_abbreviation,
...
def convert(
    values, source_measure_or_unit_abbreviation, target_measure_or_unit_abbreviation
):
    """
    ...
    source_dimension = get_dimension_by_unit_measure_or_abbreviation(
        source_measure_or_unit_abbreviation
    )
    target_dimension = get_dimension_by_unit_measure_or_abbreviation(
        target_measure_or_unit_abbreviation
    )
    ...
    if source_dimension == target_dimension:
        source = JSONObject({})
        target = JSONObject({})
        source.unit_abbreviation, source.factor = _parse_unit(
            source_measure_or_unit_abbreviation
        )
        target.unit_abbreviation, target.factor = _parse_unit(
            target_measure_or_unit_abbreviation
        )
        ...
        source.conv_factor = JSONObject(
            {"lf": source.unit_data.lf, "cf": source.unit_data.cf}
        )
        target.conv_factor = JSONObject(
            {"lf": target.unit_data.lf, "cf": target.unit_data.cf}
        )

        if isinstance(values, float):
            # If values is a float => returns a float
            return (
                source.conv_factor.lf / target.conv_factor.lf * (source.factor * values)
                + (source.conv_factor.cf - target.conv_factor.cf)
                / target.conv_factor.lf
            ) / target.factor
        elif isinstance(values, list):
            # If values is a list of floats => returns a list of floats
            return [
                (
                    source.conv_factor.lf
                    / target.conv_factor.lf
                    * (source.factor * value)
                    + (source.conv_factor.cf - target.conv_factor.cf)
                    / target.conv_factor.lf
...
def get_dimension(dimension_id, do_accept_dimension_id_none=False, **kwargs):
    ...
    try:
        dimension = (
            db.DBSession.query(Dimension).filter(Dimension.id == dimension_id).one()
        )
...
def get_dimensions(**kwargs):
    ...
    """
    dimensions_list = (
        db.DBSession.query(Dimension).options(load_only(Dimension.id)).all()
    )
    return_list = []
...
def get_dimension_by_name(dimension_name, **kwargs):
    ...
    try:
        ...
        dimension = (
            ...
            .filter(func.lower(Dimension.name) == func.lower(dimension_name.strip()))
            .one()
        )
...
def get_dimension_by_unit_measure_or_abbreviation(
    measure_or_unit_abbreviation, **kwargs
):
    """
    ...
    units = (
        db.DBSession.query(Unit).filter(Unit.abbreviation == unit_abbreviation).all()
    )
    ...
    if len(units) == 0:
        ...
        raise HydraError(
            "Unit %s has multiple dimensions not found." % (unit_abbreviation)
        )
    else:
        dimension = (
            db.DBSession.query(Dimension)
            .filter(Dimension.id == units[0].dimension_id)
            .one()
        )
        return dimension
...
def get_unit_by_abbreviation(unit_abbreviation, **kwargs):
    ...
    try:
        if unit_abbreviation is None:
            unit_abbreviation = ""
        unit_i = (
            db.DBSession.query(Unit)
            .filter(Unit.abbreviation == unit_abbreviation.strip())
            .one()
        )
...
def add_dimension(dimension, **kwargs):
    ...
    db_dimension = (
        db.DBSession.query(Dimension)
        .filter(Dimension.name == dimension["name"])
        .first()
    )

    if db_dimension is not None:
        raise HydraError(
            f"A dimension with name {dimension['name']} "
    ...
    # Load all the record
    db_dimension = (
        db.DBSession.query(Dimension).filter(Dimension.id == new_dimension.id).one()
    )
    # lazy load units
    db_dimension.units
...
def update_dimension(dimension, **kwargs):
    ...
    try:
        db_dimension = (
            db.DBSession.query(Dimension)
            .filter(Dimension.id == dimension.id)
            .filter()
            .one()
        )
        # laxy load units
        ...
        if (
            "project_id" in dimension
            and dimension["project_id"] is not None
            and dimension["project_id"] != ""
            and dimension["project_id"].isdigit()
        ):
...
def delete_dimension(dimension_id, **kwargs):
    ...
    try:
        ...
        raise ResourceNotFoundError(
            "Dimension (dimension_id=%s) does not exist" % (dimension_id)
        )
...
def update_unit(unit, **kwargs):
    ...
    try:
        ...
        if (
            "project_id" in unit
            and unit["project_id"] is not None
            and unit["project_id"] != ""
        ):
...
def check_unit_matches_dimension(unit_id, dimension_id, **kwargs):
    ...
    if unit_i.dimension.id != dimension_id:
        raise ValidationError(
            f"Unit {unit_i.name} has a dimension of {unit_i.dimension.name}, not {dimension_name}"
        )
...
def convert_dataset(dataset_id, target_unit_abbreviation, **kwargs):
    ...
    if source_unit_abbreviation is not None:
        if dataset_type == "scalar":
            new_val = convert(
                float(dsval), source_unit_abbreviation, target_unit_abbreviation
            )
        elif dataset_type == "array":
            dim = array_dim(dsval)
            vecdata = arr_to_vector(dsval)
            newvec = convert(
                vecdata, source_unit_abbreviation, target_unit_abbreviation
            )
        ...
        existing_ds = (
            db.DBSession.query(Dataset).filter(Dataset.hash == new_dataset.hash).first()
        )
...
def validate_resource_attributes(
    resource, attributes, template, check_unit=True, exact_match=False, **kwargs
):
    """
    ...
    *exact_match*: Ensures that all the attributes in the template are in
    ...
    if res_user_type is None:
        ...
        errors.append(
            "Resource %s is defined as having type %s but "
    ...
    if exact_match is True:
        ...
        if len(in_tmpl_not_in_resource) > 0:
            errors.append(
                "Template has defined attributes %s for type %s but they are not"
                " specified in the Data."
                % (",".join(in_tmpl_not_in_resource), res_user_type)
            )

        if len(in_resource_not_in_tmpl) > 0:
            errors.append(
                "Resource %s (type %s) has defined attributes %s but this is not"
                " specified in the Template."
                % (resource["name"], res_user_type, ",".join(in_resource_not_in_tmpl))
            )
    ...
    for res_attr in resource["attributes"]:
        ...
        if attr is None:
            errors.append(
                "An attribute mismatch has occurred. Attr %s is not "
        ...
        if tmpl_attrs.get(attr["name"]) is None:
            errors.append(
                "Resource %s has defined attribute %s but this is not"
            ...
            if tmpl_attr.get("data_type") is not None:
                if res_attr.get("type") is not None:
                    if tmpl_attr.get("data_type") != res_attr.get("type"):
                        errors.append(
                            "Error in data. Template says that %s on %s is a %s, but data suggests it is a %s"
                            % (
                                attr["name"],
                                resource["name"],
                                tmpl_attr.get("data_type"),
                                res_attr.get("type"),
                            )
                        )
            ...
            attr_dimension = (
                "dimensionless"
                if attr.get("dimension") is None
                else attr.get("dimension")
            )
            tmpl_attr_dimension = (
                "dimensionless"
                if tmpl_attr.get("dimension") is None
                else tmpl_attr.get("dimension")
            )

            if attr_dimension.lower() != tmpl_attr_dimension.lower():
                errors.append(
                    "Dimension mismatch on resource %s for attribute %s"
                    " (template says %s on type %s, data says %s)"
                    % (
                        resource["name"],
                        attr.get("name"),
                        tmpl_attr.get("dimension"),
                        res_user_type,
                        attr_dimension,
                    )
                )
            ...
            if check_unit is True:
                if tmpl_attr.get("unit") is not None:
                    if attr.get("unit") != tmpl_attr.get("unit"):
                        errors.append(
                            "Unit mismatch for resource %s with unit %s "
