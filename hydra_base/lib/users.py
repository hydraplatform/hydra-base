...
def delete_user(deleted_user_id, **kwargs):
    ...
    try:
        ...
        raise ResourceNotFoundError(
            "User (user_id=%s) does not exist" % (deleted_user_id)
        )
...
def get_remaining_login_attempts(username, **kwargs):
    ...
    log.info(
        "Max login attempts: %s, Failed logins: %s", max_login_attempts, failed_attempts
    )
...
def inc_failed_login_attempts(username, flush=True, **kwargs):
    ...
    log.info(
        "[ inc_failed_login_attempts ] User {} has currently {} failed logins".format(
            username, user_i.failed_logins
        )
    )
    if flush is True:
        db.DBSession.flush()
...
def delete_user_role(deleted_user_id, role_id, **kwargs):
    ...
    try:
        ...
        roleuser_i = (
            ...
            .filter(RoleUser.user_id == deleted_user_id, RoleUser.role_id == role_id)
            .one()
        )
...
def delete_role_perm(role_id, perm_id, **kwargs):
    ...
    try:
        roleperm_i = (
            ...
            .filter(RolePerm.role_id == role_id, RolePerm.perm_id == perm_id)
            .one()
        )
...
def update_role(role, **kwargs):
    ...
    for user in role.users:
        _get_user(user.id)
        roleuser_i = RoleUser(user_id=user.id, perm_id=perm.id)
    ...
    if filter_type is not None:
        ...
        if filter_type == "id":
            ...
            if isinstance(filter_value, str):
                # Trying to read a csv string
                log.info(
                    "[HB.users] Getting user by Filter Username : %s", filter_value
                )
...
def get_user_roles(uid, **kwargs):
    ...
    try:
        user_roles = (
            db.DBSession.query(Role)
            .filter(Role.id == RoleUser.role_id, RoleUser.user_id == uid)
            .all()
        )
        return user_roles
    except NoResultFound:
...
def get_user_permissions(uid, **kwargs):
    ...
    try:
        ...
        user_perms = (
            db.DBSession.query(Perm)
            .filter(
                Perm.id == RolePerm.perm_id,
...
def get_perm_by_code(perm_code, **kwargs):
    ...
    try:
        ...
        raise ResourceNotFoundError(
            "Permission not found (perm_code={})".format(perm_code)
        )
