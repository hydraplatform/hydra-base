...
def _get_project(project_id, user_id, check_write=False):
    try:
        project_i = (
            ...
            .options(noload(Project.children))
            .one()
        )
...
def add_project(project, **kwargs):
    ...
    if len(existing_proj) > 0:
        if existing_proj[0].status == "X":
            ...
            log.info(
                "Updating an existing deleted project %s with new project name to avoid naming clash %s",
                existing_proj[0].id,
                existing_proj[0].name,
            )
        else:
...
def update_project(project, **kwargs):
    ...
    if project.attributes:
        attr_map = hdb.add_resource_attributes(proj_i, project.attributes)
        proj_data = _add_project_attribute_data(
            proj_i, attr_map, project.attribute_data
        )
        proj_i.attribute_data = proj_data
...
def get_project(project_id, include_deleted_networks=False, **kwargs):
    ...
    proj_j.networks = proj_i.get_networks(
        user_id, include_deleted_networks=include_deleted_networks
    )
    ...
    **kwargs,
    ...
    if include_shared_projects is True:
        projects_qry = projects_qry.outerjoin(ProjectOwner).filter(
            Project.status == "A", ProjectOwner.user_id == uid, ProjectOwner.view == "Y"
        )
    else:
    ...
    if projects_ids_list_filter is not None:
        ...
        if isinstance(projects_ids_list_filter, str):
            ...
            if isinstance(projects_ids_list_filter, int):
                ...
                projects_qry = projects_qry.filter(
                    Project.id.in_(projects_ids_list_filter)
                )
    ...
    nav_project_ids = (
        set(Project.get_cache(uid).get(project_id, [])) - scoped_project_ids
    )
    nav_projects_i = (
        db.DBSession.query(Project).filter(Project.id.in_(nav_project_ids)).all()
    )
    nav_projects = []
    ...
    project_network_lookup = get_projects_networks(
        [p.id for p in projects_i], uid, isadmin=isadmin, **kwargs
    )
    ...
    for project_i in scoped_projects:
        ...
        project_j.projects = [
            JSONObject(p) for p in project_i.get_child_projects(req_user_id)
        ]
...
def get_projects_in(project_ids, **kwargs):
    ...
    projects = db.DBSession.query(Project).filter(Project.id.in_(project_ids))
...
def get_projects_networks(project_ids, uid, isadmin=None, **kwargs):
    ...
    network_qry = (
        ...
        .filter(Network.project_id.in_(project_ids), Network.status == "A")
    )
    if not isadmin:
        network_qry.outerjoin(NetworkOwner).filter(
            NetworkOwner.user_id == uid, NetworkOwner.view == "Y"
        )
...
def get_networks(project_id, include_data="N", **kwargs):
    ...
    rs = (
        db.DBSession.query(Network.id, Network.status)
    ...
    for r in rs:
        ...
        try:
            net = network.get_network(
                r.id, summary=True, include_data=include_data, **kwargs
            )
            log.info("Network %s retrieved", net.name)
            networks.append(net)
        except PermissionError:
            log.info(
                "Not returning network %s as user %s does not have "
...
def clone_project(
    ...
    **kwargs,
    ...
    if new_project_name is None:
        new_project_name = project.name + " Cloned By {}".format(
            cloning_user.display_name
        )
    ...
    project_with_name = (
        db.DBSession.query(Project)
        .filter(
            Project.name == new_project_name, Project.created_by == recipient_user_id
        )
        .first()
    )
    ...
    if project_with_name is not None:
        ...
        if project_with_name.status == "X":
            ...
            log.info(
                "Updating an existing deleted project %s with new project anme to avoid naming clash %s",
                project_with_name.id,
                project_with_name.name,
            )
        else:
    ...
    network_ids = (
        db.DBSession.query(Network.id).filter(Network.project_id == project_id).all()
    )
...
def _get_project_hierarchy(project_id, user_id):
    ...
    if proj.parent_id:
        project_hierarchy = project_hierarchy + _get_project_hierarchy(
            proj.parent_id, user_id
        )
