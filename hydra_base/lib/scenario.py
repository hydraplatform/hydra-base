...
def _get_scenario(scenario_id, user_id, check_write=False, check_can_edit=False):
    ...
    try:
        scenario_i = (
            db.DBSession.query(Scenario).filter(Scenario.id == scenario_id).one()
        )
        ...
        if check_can_edit is True:
            if scenario_i.locked == "Y":
                raise PermissionError(
                    f"Cannot update scenario {scenario_id} as it is locked."
                )
...
def merge_scenarios(
    ...
    **kwargs,
    ...
    target_resource_lookup = dict(
        (n.name, n)
        for n in target_network.nodes
        + target_network.links
        + target_network.resourcegroups
    )
    ...
    if len(non_matching_node_names) > 0:
        if match_all_names is True:
            raise HydraError(
                f"Unable to merge scenario {source_scenario.name} ({source_scenario.id}) into {target_scenario.name} ({target_scenario.id}) as the following node names do not match: {non_matching_node_names}"
            )

        log.warning(
            f"The following nodes in scenario {source_scenario.name} ({source_scenario.id}) no not match nodes in scenario {target_scenario.name} ({target_scenario.id}) : {non_matching_node_names}"
        )

    cloned_target = clone_scenario(target_scenario.id, user_id=user_id)
    # we need the ORM object here, not a JSONObject
    cloned_scenario = (
        db.DBSession.query(Scenario).filter(Scenario.id == cloned_target.id).one()
    )

    # Make it easy to access resource scenarios from the resource attr ID with a map
    source_ra_rs_map = dict(
        (rs.resource_attr_id, rs) for rs in source_scenario.resourcescenarios
    )
    target_ra_rs_map = dict(
        (rs.resource_attr_id, rs) for rs in cloned_scenario.resourcescenarios
    )
    ...
    for source_resource in (
        [source_network]
        + source_network.nodes
        + source_network.links
        + source_network.resourcegroups
    ):
        ...
        if target_resource is None:
            log.warning(
                f"Resource '{source_resource.name}' not found on target. Ignoring."
            )
            continue

        target_attribute_map = dict(
            (a.attr_id, a.id) for a in target_resource.attributes
        )
        for source_ra in source_resource.attributes:
            ...
            if target_ra_id is None:
                if ignore_missing_attributes is True:
                    log.warning(
                        f"Unable to find attribute {source_ra.attr.name} on target {target_resource.name}. Ignoring."
                    )
                    continue
                else:
                    new_ra = ResourceAttr()
                    ...
                    log.info(
                        f"Adding new attribute {source_ra.attr.name} to {target_resource.name}"
                    )
...
@required_perms("edit_network")
def copy_data_from_scenario(
    resource_attrs, source_scenario_id, target_scenario_id, **kwargs
):
    """
    ...
    target_resourcescenarios = (
        ...
        .filter(
            ...
            ResourceScenario.resource_attr_id.in_(resource_attrs),
        )
        .all()
    )
...
def add_scenario(network_id, scenario, **kwargs):
    ...
    existing_scen = (
        db.DBSession.query(Scenario)
        .filter(Scenario.name == scenario.name, Scenario.network_id == network_id)
        .first()
    )
    if existing_scen is not None:
        raise HydraError(
            "Scenario with name %s already exists in network %s"
...
def update_scenario(
    scenario, update_data=True, update_groups=True, flush=True, **kwargs
):
    """
    ...
    if update_data is True:
        ...
        updated_datasets = data._bulk_insert_data(
            datasets, user_id, kwargs.get("app_name")
        )
...
def set_scenario_status(scenario_id, status, **kwargs):
    ...
    scenario_i = _get_scenario(
        scenario_id, user_id, check_write=True, check_can_edit=True
    )
...
def create_child_scenario(parent_scenario_id, child_name, **kwargs):
    ...
    existing_scenarios = (
        db.DBSession.query(Scenario)
        .filter(Scenario.network_id == scen_i.network_id)
        .all()
    )
    if child_name is not None:
        for existing_scenario in existing_scenarios:
            if existing_scenario.name == child_name:
                raise HydraError(
                    "A scenario with the name {0} already exists in this network. ".format(
                        child_name
                    )
                )
    else:
...
def clone_scenario(scenario_id, retain_results=False, scenario_name=None, **kwargs):
    ...
    if scenario_name is None:
        ...
        existing_scenario_with_name = (
            db.DBSession.query(Scenario)
            .filter(
                Scenario.network_id == scen_i.network_id, Scenario.name == scenario_name
            )
            .first()
        )
        ...
        if existing_scenario_with_name is not None:
            ...
            raise HydraError(
                f"A scenario with name {scenario_name} already exists in network {netname}"
            )
    ...
    new_rscen_rs = (
        db.DBSession.query(ResourceScenario)
        .filter(ResourceScenario.scenario_id == cloned_scenario_id)
        .all()
    )
    new_rgis_rs = (
        db.DBSession.query(ResourceGroupItem)
        .filter(ResourceGroupItem.scenario_id == cloned_scenario_id)
        .all()
    )
...
def _get_scenario_network_resources(scenario_id, scenario_i=None):
    ...
    net = (
        db.DBSession.query(Network.id, Network.name)
        .filter(Network.id == scenario_i.network_id)
        .all()
    )
    nodes = (
        db.DBSession.query(Node.id, Node.name)
        .filter(Node.network_id == scenario_i.network_id)
        .all()
    )
    links = (
        db.DBSession.query(Link.id, Link.name)
        .filter(Link.network_id == scenario_i.network_id)
        .all()
    )
    groups = (
        db.DBSession.query(ResourceGroup.id, ResourceGroup.name)
        .filter(ResourceGroup.network_id == scenario_i.network_id)
        .all()
    )
...
def compare_scenarios(
    scenario_id_1,
    scenario_id_2,
    allow_different_networks=False,
    ...
    **kwargs,
    ...
    if (
        allow_different_networks is False
        and scenario_1.network_id != scenario_2.network_id
    ):
        raise HydraError(
            "Cannot compare scenarios that are not" " in the same network!"
        )
    ...
    log.info(
        "Datasets In 1 not in 2: %s" % (len(rscen_1_dataset_ids - rscen_2_dataset_ids))
    )
    log.info(
        "Datasets In 2 not in 1: %s" % (len(rscen_2_dataset_ids - rscen_1_dataset_ids))
    )
        ...
        if s2_rs is not None:
            ...
            if s1_rs.dataset_id != s2_rs.dataset_id:
                resource_diff = dict(
                    ...
                    resource_name=s1_resource_mapping[s1_rs.resourceattr.ref_key][
                        _get_resource_id(s1_rs.resourceattr)
                    ],
                )
    ...
    for ra_id, s2_rs in r_scen_2_dict.items():
        ...
        if s1_rs is None:
            resource_diff = dict(
                ...
                resource_name=s2_resource_mapping[s2_rs.resourceattr.ref_key][
                    _get_resource_id(s2_rs.resourceattr)
                ],
            )
            resource_diffs.append(resource_diff)
...
def get_resource_scenario(
    resource_attr_id, scenario_id, get_parent_data=False, **kwargs
):
    """
    ...
    scenario_rs = scenario_i.get_data(
        user_id, get_parent_data=get_parent_data, ra_ids=[resource_attr_id]
    )
    ...
    for rs_i in scenario_rs:
        ...
        raise ResourceNotFoundError(
            "resource scenario for %s not found in scenario %s"
            % (resource_attr_id, scenario_id)
        )


@required_perms("get_data")
def get_resourceattr_data(
    resource_attr_ids, scenario_id, get_parent_data=False, **kwargs
):
    """
    ...
    scenario_rs = scenario_i.get_data(
        user_id, get_parent_data=get_parent_data, ra_ids=resource_attr_ids
    )
...
def get_dataset_scenarios(dataset_id, **kwargs):
    ...
    scenarios = (
        ...
        .filter(
            ...
            ResourceScenario.dataset_id == dataset_id,
        )
        .distinct()
        .all()
    )
...
def bulk_update_resourcedata(scenario_ids, resource_scenarios, **kwargs):
    ...
    net_ids = (
        db.DBSession.query(Scenario.network_id)
    ...
    for scenario_id in scenario_ids:
        ...
        r_scens_i = (
            ...
            .filter(
                ResourceScenario.scenario_id == scenario_id,
                ResourceScenario.resource_attr_id.in_(ra_ids),
            )
            .all()
        )
...
def update_resourcedata(scenario_id, resource_scenarios, **kwargs):
    ...
    for rs in resource_scenarios:
        if rs.dataset is not None:
            updated_rs = _update_resourcescenario(
                scenario_i, rs, user_id=user_id, source=kwargs.get("app_name")
            )
...
def delete_resource_scenarios(scenario_id, resource_attr_ids, quiet=False, **kwargs):
    ...
    log.info(
        "Deleting %s resource scenarios from from scenario %s",
        len(resource_attr_ids),
        scenario_id,
    )
    _get_scenario(scenario_id, user_id, check_write=True, check_can_edit=True)
...
def delete_resourcedata(scenario_id, resource_scenario, quiet=False, **kwargs):
    ...
    _delete_resourcescenario(
        scenario_id, resource_scenario.resource_attr_id, suppress_error=quiet
    )
...
def _delete_resourcescenario(scenario_id, resource_attr_id, suppress_error=False):
    ...
    log.debug(
        "Deleting resource scenario for RA %s from scenario %s",
        resource_attr_id,
        scenario_id,
    )
    ...
    try:
        ...
        if suppress_error is False:
            raise HydraError(
                "ResourceAttr %s does not exist in scenario %s."
                % (resource_attr_id, scenario_id)
            )
        return
        ...
        try:
            r_scen_i = (
                ...
                .filter(
                    ...
                    == resource_scenario.resource_attr_id,
                )
                .one()
            )
        except NoResultFound as e:
            log.info(
                "Creating new RS for RS %s in scenario %s",
                resource_scenario.resource_attr_id,
                scenario.id,
            )
...
def assign_value(
    ...
    log.debug(
        "Assigning value %s to rs %s in scenario %s",
        name,
        rs.resource_attr_id,
        rs.scenario_id,
    )
    ...
    if rs.scenario.locked == "Y":
        raise PermissionError(
            "Cannot assign value. Scenario %s is locked" % (rs.scenario_id)
        )
    ...
    if rs.dataset is not None:
        ...
        connected_rs = (
            db.DBSession.query(ResourceScenario)
            .filter(ResourceScenario.dataset_id == rs.dataset.id)
            .all()
        )
        ...
        if len(connected_rs) == 1:
            if (
                connected_rs[0].scenario_id == rs.scenario_id
                and connected_rs[0].resource_attr_id == rs.resource_attr_id
            ):
                update_dataset = True
        else:
...
def add_data_to_attribute(scenario_id, resource_attr_id, dataset, **kwargs):
    ...
    try:
        ...
        log.info(
            "Existing resource scenario found for %s in scenario %s",
            resource_attr_id,
            scenario_id,
        )
    except NoResultFound:
    ...
    if value is None:
        raise HydraError(
            f"Cannot set value to attribute. No value was sent with dataset {dataset_j.id}"
        )
...
def get_attribute_data(attr_ids, node_ids, **kwargs):
    ...
    node_attrs = (
        ...
        .filter(ResourceAttr.node_id.in_(node_ids), ResourceAttr.attr_id.in_(attr_ids))
        .all()
    )
...
def _get_all_network_resource_attributes(network_id):
    ...
    log.info(
        "%s attrs retrieved in %s",
        len(all_resource_attributes),
        round(time.time() - x, 2),
    )
...
def get_resource_data(
    ...
    **kwargs,
    ...
    if ref_key is not None and ref_id is not None:
        ...
        raise HydraError(
            "Unable to get data. Must specify a resource type (ref_key) and resource id (ref_id)"
        )
    ...
    if include_inputs is False or include_outputs is False:
        if include_inputs is False:
            ...
            resource_attributes = list(
                filter(lambda x: x.attr_is_var == "Y", resource_attributes)
            )

        if include_outputs is False:
            # only include inputs
            resource_attributes = list(
                filter(lambda x: x.attr_is_var == "N", resource_attributes)
            )
    ...
    requested_rs = scenario_i.get_data(
        user_id,
        get_parent_data=get_parent_data,
        ra_ids=ra_ids,
...
def delete_resourcegroupitems(scenario_id, item_ids, **kwargs):
    ...
    for item_id in item_ids:
        rgi = (
            db.DBSession.query(ResourceGroupItem)
            .filter(ResourceGroupItem.id == item_id)
            .one()
        )
...
def _add_resourcegroupitem(group_item, scenario_id):
    ...
    if group_item.id and group_item.id > 0:
        try:
            group_item_i = (
                db.DBSession.query(ResourceGroupItem)
                .filter(ResourceGroupItem.id == group_item.id)
                .one()
            )
        except NoResultFound:
            raise ResourceNotFoundError(
                "ResourceGroupItem %s not found" % (group_item.id)
            )
    ...
    if ref_key == "NODE":
        group_item_i.node_id = (
            group_item.ref_id if group_item.ref_id else group_item.node_id
        )
    elif ref_key == "LINK":
        group_item_i.link_id = (
            group_item.ref_id if group_item.ref_id else group_item.link_id
        )
    elif ref_key == "GROUP":
        group_item_i.subgroup_id = (
            group_item.ref_id if group_item.ref_id else group_item.subgroup_id
        )
    ...
    **kwargs,
    ...
    rs1 = (
        ...
        .filter(
            ...
            rs.scenario_id == source_scenario_id,
        )
        .first()
    )
    rs2 = (
        ...
        .filter(
            ...
            rs.scenario_id == target_scenario_id,
        )
        .first()
    )
    ...
    return_value = (
        None  # Either return null or return a new or updated resource scenario
    )
    if rs1 is not None:
        if rs2 is not None:
