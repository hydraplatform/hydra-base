...
def share_network(network_id, usernames, read_only, share, **kwargs):
    ...
    if net_i.created_by != int(user_id) and share == "Y":
        raise HydraError(
            f"Cannot share the 'sharing' ability as user {user_id} is not"
...
def share_project(
    project_id, usernames, read_only=False, share=False, flush=True, **kwargs
):
    """
...
def set_project_permission(project_id, usernames, read, write, share, **kwargs):
    ...
    for username in usernames:
        ...
        if proj_i.created_by == user_i.id:
            raise HydraError(
                "Cannot set permissions on project %s"
                " for user %s as this user is the creator." % (project_id, username)
            )
...
def set_network_permission(network_id, usernames, read, write, share, **kwargs):
    ...
    for username in usernames:
        ...
        if net_i.created_by == user_i.id:
            raise HydraError(
                "Cannot set permissions on network %s"
                " for user %s as tis user is the creator." % (network_id, username)
            )
...
def hide_dataset(dataset_id, exceptions, read, write, share, **kwargs):
    ...
    if dataset_i.created_by != int(user_id):
        raise HydraError(
            "Permission denied. "
            "User %s is not the owner of dataset %s" % (user_id, dataset_i.name)
        )
...
def get_all_network_owners(network_ids=None, **kwargs):
    ...
    if network_ids is not None:
        networkowner_qry = networkowner_qry.filter(
            NetworkOwner.network_id.in_(network_ids)
        )
...
def bulk_set_project_owners(project_owners, **kwargs):
    ...
    existing_projowners = (
        db.DBSession.query(ProjectOwner)
        .filter(ProjectOwner.project_id.in_(project_ids))
        .all()
    )
...
def bulk_set_network_owners(network_owners, **kwargs):
    ...
    existing_projowners = (
        db.DBSession.query(NetworkOwner)
        .filter(NetworkOwner.network_id.in_(network_ids))
        .all()
    )
