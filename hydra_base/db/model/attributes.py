...
class Attr(Base, Inspect):
    ...
    network = relationship(
        ...
        backref=backref("scopedattributes", uselist=True, cascade="all, delete-orphan"),
        lazy="joined",
    )
    ...
    dimension = relationship(
        ...
        backref=backref("attributes", uselist=True),
    )
...
class AttrMap(Base, Inspect):
    ...
    attr_id_a = Column(
        Integer(), ForeignKey("tAttr.id"), primary_key=True, nullable=False
    )
    attr_id_b = Column(
        Integer(), ForeignKey("tAttr.id"), primary_key=True, nullable=False
    )
    ...
    attr_a = relationship(
        "Attr", foreign_keys=[attr_id_a], backref=backref("maps_to", order_by=attr_id_a)
    )
    attr_b = relationship(
        "Attr",
        foreign_keys=[attr_id_b],
        backref=backref("maps_from", order_by=attr_id_b),
    )
...
class AttrGroup(Base, Inspect):
    ...
    **exclusive** : If 'Y' then an attribute in this group cannot be in any other groups
    ...
    exclusive = Column(String(1), nullable=False, server_default=text("'N'"))
    project_id = Column(
        Integer(), ForeignKey("tProject.id"), primary_key=False, nullable=False
    )
    cr_date = Column(
        TIMESTAMP(), nullable=False, server_default=text("CURRENT_TIMESTAMP")
    )
    ...
    project = relationship(
        "Project",
        backref=backref("attrgroups", uselist=True, cascade="all, delete-orphan"),
        lazy="joined",
    )
...
class AttrGroupItem(Base, Inspect):
    ...
    group_id = Column(
        Integer(), ForeignKey("tAttrGroup.id"), primary_key=True, nullable=False
    )
    attr_id = Column(
        Integer(), ForeignKey("tAttr.id"), primary_key=True, nullable=False
    )
    ...
    group = relationship(
        "AttrGroup",
        backref=backref("items", uselist=True, cascade="all, delete-orphan"),
        lazy="joined",
    )
    attr = relationship("Attr")
    network = relationship(
        "Network",
        backref=backref("attrgroupitems", uselist=True, cascade="all, delete-orphan"),
        lazy="joined",
    )
