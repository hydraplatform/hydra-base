...
class ResourceScenario(Base, Inspect):
    ...
    scenario_id = Column(
        Integer(),
        ForeignKey("tScenario.id"),
        primary_key=True,
        nullable=False,
        index=True,
    )
    resource_attr_id = Column(
        Integer(),
        ForeignKey("tResourceAttr.id"),
        primary_key=True,
        nullable=False,
        index=True,
    )
    ...
    dataset = relationship(
        "Dataset", backref=backref("resourcescenarios", order_by=dataset_id)
    )
    scenario = relationship(
        "Scenario",
        backref=backref(
            "resourcescenarios", order_by=scenario_id, cascade="all, delete-orphan"
        ),
    )
    resourceattr = relationship(
        "ResourceAttr",
        backref=backref("resourcescenarios", cascade="all, delete-orphan"),
        uselist=False,
    )
    ...
    def get_dataset(self, user_id):
        dataset = (
            ...
            .outerjoin(
                DatasetOwner,
                and_(
                    Dataset.id == DatasetOwner.dataset_id,
...
class Scenario(Base, Inspect):
    ...
    locked = Column(String(1), nullable=False, server_default=text("'N'"))
    ...
    parent = relationship(
        "Scenario", remote_side=[id], backref=backref("children", order_by=id)
    )
    ...
    def get_all_resourcescenarios(
        ...
        if include_data_types is not None:
            rs_qry = rs_qry.filter(
                func.upper(Dataset.type).in_([t.upper() for t in include_data_types])
            )

        if exclude_data_types is not None:
            rs_qry = rs_qry.filter(
                func.upper(Dataset.type).not_in([t.upper() for t in exclude_data_types])
            )
        ...
        if include_values is True:
            if include_data_type_values is not None:
                rs_qry = rs_qry.filter(
                    func.upper(Dataset.type).in_(
                        [t.upper() for t in include_data_type_values]
                    )
                )

            if exclude_data_type_values is not None:
                excl_values_qry = rs_qry.filter(
                    func.upper(Dataset.type).in_(
                        [t.upper() for t in exclude_data_type_values]
                    )
                )
                rs_qry = rs_qry.filter(
                    func.upper(Dataset.type).not_in(
                        [t.upper() for t in exclude_data_type_values]
                    )
                )
        ...
        if include_metadata is True:
            ...
            metadata = (
                ...
                .filter(ResourceScenario.scenario_id == self.id)
                .all()
            )
            for m in metadata:
    ...
    def get_group_items(self, child_items=None, get_parent_items=False):
        ...
        if self.parent is not None and get_parent_items is True:
            return self.parent.get_group_items(
                child_items=child_items, get_parent_items=get_parent_items
            )
