...
class Rule(AuditMixin, Base, Inspect, PermissionControlled):
    ...
    __tablename__ = "tRule"
    __table_args__ = (UniqueConstraint("scenario_id", "name", name="unique rule name"),)
    ...
    scenario = relationship(
        "Scenario",
        backref=backref("rules", uselist=True, cascade="all, delete-orphan"),
        lazy="joined",
    )
    network = relationship(
        "Network",
        backref=backref("rules", order_by=network_id, cascade="all, delete-orphan"),
        lazy="joined",
    )
    node = relationship(
        "Node",
        backref=backref(
            "rules", order_by=node_id, uselist=True, cascade="all, delete-orphan"
        ),
        lazy="joined",
    )
    link = relationship(
        "Link",
        backref=backref(
            "rules", order_by=link_id, uselist=True, cascade="all, delete-orphan"
        ),
        lazy="joined",
    )
    group = relationship(
        "ResourceGroup",
        backref=backref(
            "rules", order_by=group_id, uselist=True, cascade="all, delete-orphan"  #
        ),
        lazy="joined",
    )
    ...
    def check_type_definition_exists(self, code):
        ...
        try:
            get_session().query(RuleTypeDefinition).filter(
                RuleTypeDefinition.code == code
            ).one()
        except NoResultFound:
            raise ResourceNotFoundError(
                "Rule type definition with code {} does not exist".format(code)
            )
    ...
    def get_owners(self):
        ...
        network_owners = list(
            filter(lambda x: x.user_id not in owner_ids, network.get_owners())
        )
...
class RuleTypeDefinition(AuditMixin, Base, Inspect):
    ...
    __table_args__ = (UniqueConstraint("code", name="Unique Rule Code"),)
...
class RuleTypeLink(AuditMixin, Base, Inspect):
    ...
    __table_args__ = (UniqueConstraint("code", "rule_id", name="Unique Rule / Type"),)

    code = Column(
        String(200),
        ForeignKey("tRuleTypeDefinition.code"),
        primary_key=True,
        nullable=False,
    )
    rule_id = Column(
    ...
    rule = relationship(
        "Rule",
        backref=backref(
            "types", order_by=code, uselist=True, cascade="all, delete-orphan"
        ),
    )
