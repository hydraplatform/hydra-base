...
class Network(Base, Inspect, PermissionControlled, Resource):
    ...
    __table_args__ = (UniqueConstraint("name", "project_id", name="unique net name"),)
    ref_key = "NETWORK"
    ...
    project = relationship(
        "Project",
        backref=backref(
            "networks", order_by="asc(Network.cr_date)", cascade="all, delete-orphan"
        ),
    )
    ...
    def add_link(self, name, desc, layout, node_1, node_2):
        ...
        existing_link = (
            get_session()
            .query(Link)
            .filter(Link.name == name, Link.network_id == self.id)
            .first()
        )
        if existing_link is not None:
            raise HydraError(
                "A link with name %s is already in network %s" % (name, self.id)
            )
    ...
    def add_node(self, name, desc, layout, node_x, node_y):
        ...
        existing_node = (
            get_session()
            .query(Node)
            .filter(Node.name == name, Node.network_id == self.id)
            .first()
        )
        if existing_node is not None:
            raise HydraError(
                "A node with name %s is already in network %s" % (name, self.id)
            )
    ...
    def add_group(self, name, desc, status):
        ...
        existing_group = (
            get_session()
            .query(ResourceGroup)
            .filter(ResourceGroup.name == name, ResourceGroup.network_id == self.id)
            .first()
        )
        if existing_group is not None:
            raise HydraError(
                "A resource group with name %s is already in network %s"
                % (name, self.id)
            )
    ...
    def unset_owner(self, user_id):
        ...
        if str(user_id) == str(self.created_by):
            log.warning(
                "Cannot unset %s as owner, as they created the network", user_id
            )
            return
        for o in self.owners:
    ...
    def check_read_permission(self, user_id, do_raise=True, is_admin=None):
        ...
        can_read = super(Network, self).check_read_permission(
            user_id, do_raise=False, is_admin=is_admin
        )
        ...
        if can_read is False and do_raise is True:
            raise PermissionError(
                ...
                " access on network %s" % (user_id, self.id)
            )
    ...
    def check_write_permission(self, user_id, do_raise=True, is_admin=None):
        ...
        """
        can_write = super(Network, self).check_write_permission(
            user_id, do_raise=False, is_admin=is_admin
        )
    ...
    def get_owners(self):
        ...
        project_owners = list(
            filter(lambda x: x.user_id not in owner_ids, self.project.get_owners())
        )
    ...
    def get_scoped_attributes(
        ...
        if include_hierarchy is True:
            scoped_attrs.extend(
                self.project.get_scoped_attributes(
