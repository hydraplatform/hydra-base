...
class ResourceAttr(Base, Inspect):
    ...
    attr_is_var = Column(String(1), nullable=False, server_default=text("'N'"))
    cr_date = Column(
    ...
    project = relationship(
        "Project",
        backref=backref("attributes", uselist=True, cascade="all, delete-orphan"),
        uselist=False,
    )
    ...
    def check_read_permission(self, user_id, do_raise=True, is_admin=None):
        ...
        """
        return self.get_resource().check_read_permission(
...
class ResourceAttrMap(Base, Inspect):
    ...
    resource_attr_id_a = Column(
        Integer(), ForeignKey("tResourceAttr.id"), primary_key=True, nullable=False
    )
    resource_attr_id_b = Column(
        Integer(), ForeignKey("tResourceAttr.id"), primary_key=True, nullable=False
    )
