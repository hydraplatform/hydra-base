...
class Project(Base, Inspect, PermissionControlled):
    ...
    parent = relationship(
        "Project", remote_side=[id], backref=backref("children", order_by=id)
    )
    ...
    def get_networks(self, user_id, include_deleted_networks=False):
        ...
        if self.is_owner(user_id):
            ...
            networks_not_creator = (
                get_session().query(Network).filter(Network.project_id == self.id).all()
            )
        else:
        ...
        all_owners = (
            get_session()
            .query(NetworkOwner)
            .filter(NetworkOwner.network_id.in_(all_network_ids))
            .all()
        )
        ...
        all_scenarios = (
            get_session()
            .query(Scenario)
            .filter(Scenario.network_id.in_(all_network_ids))
            .all()
        )
    ...
    def get_child_projects(self, user_id, include_deleted_networks=False, levels=1):
        ...
        child_projects_i = (
            get_session()
            .query(Project)
            .outerjoin(ProjectOwner)
            .filter(Project.parent_id == self.id)
            .all()
        )
        ...
        owners = (
            get_session()
            .query(User.id.label("user_id"), User.display_name, ProjectOwner.project_id)
            .filter(User.id == ProjectOwner.user_id)
            .filter(ProjectOwner.project_id.in_([p.id for p in child_projects_i]))
            .all()
        )

        creators = (
            get_session()
            .query(User.id.label("user_id"), User.display_name)
            .filter(User.id.in_([p.created_by for p in child_projects_i]))
            .all()
        )
    ...
    def get_owners(self):
        ...
        if self.parent_id is not None:
            parent_owners = list(
                filter(lambda x: x.user_id not in owner_ids, self.parent.get_owners())
            )
    ...
    def build_user_cache(cls, uid):
        ...
        network_projects_i = (
            network_project_qry.outerjoin(Network)
            .outerjoin(NetworkOwner)
            .filter(
                Network.status == "A",
        ...
        cls._build_user_cache_down_tree(
            uid, [p.id for p in projects_i], project_user_cache
        )
    ...
    def _build_user_cache_up_tree(cls, uid, project_ids, project_user_cache):
        ...
        projects = (
            get_session().query(Project).filter(Project.id.in_(project_ids)).all()
        )
    ...
    def get_attribute_data(self):
        attribute_data_rs = (
            get_session()
            .query(ResourceScenario)
            .join(ResourceAttr)
            .filter(ResourceAttr.project_id == self.id)
            .all()
        )
        # lazy load datasets
    ...
    def unset_owner(self, user_id):
        ...
        if str(user_id) == str(self.created_by):
            log.warning(
                "Cannot unset %s as owner, as they created the project", user_id
            )
            return
        for o in self.owners:
            if user_id == o.user_id:
    ...
    def check_read_permission(self, user_id, do_raise=True, is_admin=None, nav=True):
        ...
        has_permission = super(Project, self).check_read_permission(
            user_id, is_admin=is_admin, do_raise=False
        )
        ...
        if has_permission is False and self.parent_id is not None:
            ...
            """
            has_permission = self.parent.check_read_permission(
                user_id, nav=False, do_raise=False
            )
    ...
    def check_write_permission(self, user_id, do_raise=True, is_admin=None):
        ...
        """
        has_permission = super(Project, self).check_write_permission(
            user_id, is_admin=is_admin, do_raise=False
        )
    ...
    def get_scoped_attributes(
        self, include_hierarchy=False, name_match=None, return_json=True
    ):
        """
        ...
        if self.parent_id is not None and include_hierarchy is True:
            scoped_attrs.extend(
                self.parent.get_scoped_attributes(
