...
class Template(Base, Inspect):
    ...
    parent = relationship(
        "Template", remote_side=[id], backref=backref("children", order_by=id)
    )
    ...
    def set_inherited_columns(self, parent, child, table):
        ...
        for column in table.__table__.columns:
            ...
            if (
                hasattr(table, "_protected_columns")
    ...
    def get_typeattr(self, typeattr_id, child_typeattr=None, get_parent_types=True):
        ...
        if child_typeattr is None:
            ...
            child_typeattr = self.set_inherited_columns(
                this_typeattr, child_typeattr, this_typeattr_i
            )

        if this_typeattr.parent_id is not None and get_parent_types is True:
            return self.parent.get_typeattr(
                this_typeattr.parent_id,
        ...
        if child_type is not None:
            ...
            for i, typeattr in enumerate(typeattrs):
                if typeattr.attr_id in child_typeattrs:
                    log.debug(
                        "Found a typeattr for attribute %s on the "
                ...
                if child_typeattr is None:
                    ...
                    child_typeattr = self.set_inherited_columns(
                        typeattr, child_typeattr, typeattrs_i[i]
                    )
    ...
    def get_types(
        ...
        for i, this_type in enumerate(types):
            ...
            if this_type.id in type_tree:
                ...
                child_type = self.set_inherited_columns(
                    this_type, child_type, types_i[i]
                )
                ...
                for typeattr in typeattrs:
                    ...
                    if child_typeattr is None:
                        ...
                        if typeattr.parent_id is not None:
                            # it has a parent, so add it to the type's tree dict
                            # for processing farther up the tree
                            type_tree[this_type.id].ta_tree[
                                typeattr.parent_id
                            ] = typeattr
                        child_type.typeattrs.append(typeattr)
                    else:
                        child_typeattr = self.set_inherited_columns(
                            typeattr, child_typeattr, types_i[i]
                        )
...
class TemplateType(Base, Inspect):
    ...
    __table_args__ = (
        UniqueConstraint(
            "template_id", "name", "resource_type", name="unique type name"
        ),
    )
    ...
    parent = relationship(
        "TemplateType", remote_side=[id], backref=backref("children", order_by=id)
    )

    template = relationship(
        "Template",
        backref=backref("templatetypes", order_by=id, cascade="all, delete-orphan"),
    )
    ...
    def get_typeattrs(self, ta_tree={}, child_typeattrs=None, get_parent_types=True):
        ...
        for i, typeattr in enumerate(typeattrs):
            ...
            if child_typeattr is None:
                ...
                child_typeattr = self.template.set_inherited_columns(
                    typeattr, child_typeattr, typeattrs_i[i]
                )
                child_typeattrs.append(typeattr)

        if self.parent is not None and get_parent_types is True:
            return self.parent.get_typeattrs(
                ta_tree=ta_tree,
    ...
    def get_children(self):
        ...
        child_types = (
            get_session()
            .query(TemplateType)
            .filter(TemplateType.parent_id == self.id)
            .all()
        )
        return child_types
    ...
    def check_can_delete_resourcetypes(self, delete_resourcetypes=False):
        ...
        resourcetype_count = (
            ...
            .filter(ResourceType.type_id == self.id)
            .count()
        )

        if resourcetype_count > 0 and delete_resourcetypes is False:
            raise HydraError(
                f"Unable to delete type. Template Type {self.id} has "
    ...
    def delete_children(self, delete_resourcetypes=False):
        ...
        for child in children:
            ...
            child.check_can_delete_resourcetypes(
    ...
    def delete_resourcetypes(self):
        ...
        log.warning(
            "Forcing the deletion of %s resource types from type %s",
            len(type_rs),
            self.id,
        )
...
class TypeAttr(Base, Inspect):
    ...
    __table_args__ = (UniqueConstraint("type_id", "attr_id", name="type_attr_1"),)

    id = Column(Integer(), primary_key=True, nullable=False)
    parent_id = Column(Integer(), ForeignKey("tTypeAttr.id"))
    attr_id = Column(Integer(), ForeignKey("tAttr.id"), nullable=False)
    type_id = Column(
        Integer(), ForeignKey("tTemplateType.id", ondelete="CASCADE"), nullable=False
    )
    default_dataset_id = Column(Integer(), ForeignKey("tDataset.id"))
    attr_is_var = Column(String(1), server_default=text("'N'"))
    ...
    parent = relationship(
        "TypeAttr", remote_side=[id], backref=backref("children", order_by=id)
    )
    ...
    templatetype = relationship(
        "TemplateType", backref=backref("typeattrs", order_by=attr_id)
    )
...
class ResourceType(Base, Inspect):
    ...
    id = Column(Integer, primary_key=True, nullable=False)
    type_id = Column(
        Integer(), ForeignKey("tTemplateType.id"), primary_key=False, nullable=False
    )
    ...
    child_template_id = Column(
        Integer(), ForeignKey("tTemplate.id"), primary_key=False, nullable=True
    )
    ...
    # of the type
    templatetype = relationship(
        "TemplateType", backref=backref("resourcetypes", uselist=True)
    )

    network = relationship(
        "Network",
        backref=backref("types", uselist=True, cascade="all, delete-orphan"),
        uselist=False,
    )
    node = relationship(
        "Node",
    ...
    def get_templatetype(self):
        ...
        if self.child_template_id is not None:
            template_i = (
                get_session()
                .query(Template)
                .filter(Template.id == self.child_template_id)
                .one()
            )
        else:
            type_i = (
                get_session()
                .query(TemplateType)
                .filter(TemplateType.id == self.type_id)
                .one()
            )
            ...
            template_i = (
                get_session()
                .query(Template)
                .filter(Template.id == type_i.template_id)
                .one()
            )
