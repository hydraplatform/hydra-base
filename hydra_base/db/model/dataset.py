...
from hydra_base.lib.storage import MongoDatasetManager, MongoStorageAdapter
...
class Dataset(Base, Inspect, PermissionControlled, AuditMixin):
    ...
    hidden = Column(String(1), nullable=False, server_default=text("'N'"))
    value_ref = Column(
        "value", Text().with_variant(mysql.LONGTEXT, "mysql"), nullable=True
    )
    ...
    def set_metadata(self, metadata_tree):
        ...
        **metadata_tree**: A dictionary of metadata key-vals.
        ...
        metadata_to_delete = set(existing_metadata).difference(
            set(metadata_tree.keys())
        )
        """
    ...
    def is_external(self):
        ...
        for datum in self.metadata:
            if (
                datum.key == mongo_storage_location_key
                and datum.value == mongo_external
            ):
                return True
...
class DatasetCollectionItem(Base, Inspect):
    ...
    collection_id = Column(
        Integer(), ForeignKey("tDatasetCollection.id"), primary_key=True, nullable=False
    )
    dataset_id = Column(
    ...
    collection = relationship(
        "DatasetCollection",
        backref=backref("items", order_by=dataset_id, cascade="all, delete-orphan"),
    )
    dataset = relationship(
        "Dataset",
        backref=backref(
            "collectionitems", order_by=dataset_id, cascade="all, delete-orphan"
        ),
    )
...
class Metadata(Base, Inspect):
    ...
    dataset_id = Column(
        Integer(),
        ForeignKey("tDataset.id", ondelete="CASCADE"),
        primary_key=True,
        nullable=False,
        index=True,
    )
    ...
    dataset = relationship(
        "Dataset",
        backref=backref("metadata", order_by=dataset_id, cascade="all, delete-orphan"),
    )
