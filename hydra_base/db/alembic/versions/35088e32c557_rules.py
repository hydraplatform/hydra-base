    ...
    if op.get_bind().dialect.name == "mysql":
        ...
        try:
            op.add_column(
                "tRule",
                sa.Column(
                    "format", sa.String(80), nullable=True, server_default="text"
                ),
            )
            op.add_column(
                "tRule",
        ...
        try:
            op.create_table(
                ...
                sa.Column(
                    "format", sa.String(80), nullable=False, server_default="text"
                ),
                sa.Column("ref_key", sa.String(60), nullable=False, index=True),
                sa.Column(
                    "value",
                    sa.Text().with_variant(sa.mysql.LONGTEXT, "mysql"),
                    nullable=True,
                ),
                sa.Column(
                    "status",
            ...
            op.execute(
                "insert into tRule_new (id, name, description, ref_key, value, status, scenario_id, network_id, node_id, link_id, group_id, created_by, updated_by, updated_at) select id, name, description, ref_key, value, status, scenario_id, network_id, node_id, link_id, group_id, null, null, null from tRule"
            )
...
def downgrade():
    if op.get_bind().dialect.name == "mysql":
        ...
        try:
            ...
            op.execute(
                "insert into tRule_new (id, name, description, ref_key, value, status, scenario_id, network_id, node_id, link_id, group_id) select id, name, description, ref_key, value, status, scenario_id, network_id, node_id, link_id, group_id from tRule"
            )
