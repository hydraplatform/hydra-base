    ...
    if op.get_bind().dialect.name == "mysql":
        ...
        try:
            ...
            op.create_table(
                ...
                sa.UniqueConstraint(
                    "name", "created_by", "status", name="unique project name"
                ),
            )

            op.execute(
                "insert into tProject_new (id, name, description, status, cr_date, created_by) select project_id, project_name, project_description, status, cr_date, created_by from tProject"
            )
    ...
    if op.get_bind().dialect.name == "mysql":
        ...
        try:
            ...
            op.alter_column(
                "tProject",
                "description",
                new_column_name="project_description",
                existing_type=sa.String(1000),
            )
        except Exception as e:
            log.exception(e)
        ...
        try:
            ...
            op.execute(
                "insert into tProject_new (project_id, project_name, project_description, status, cr_date, created_by) select id, name, description, status, cr_date, created_by from tProject"
            )
