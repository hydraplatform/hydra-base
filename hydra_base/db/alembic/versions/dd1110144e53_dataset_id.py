    ...
    if op.get_bind().dialect.name == "mysql":
        ...
        try:
            ...
            op.alter_column(
                ...
                existing_type=sa.Text(1000).with_variant(mysql.TEXT(1000), "mysql"),
                nullable=False,
            )
        except Exception as e:
            log.exception(e)
        ...
        try:
            ...
            op.create_table(
                ...
                sa.Column(
                    "created_by",
                    sa.Integer(),
                    sa.ForeignKey("tUser.id"),
                    nullable=False,
                ),
            )

            op.execute(
                "insert into tDataset_new (id, name, type, unit, hash, value, cr_date, created_by) select dataset_id, data_name, data_type, data_units, data_hash, value, cr_date, created_by from tDataset"
            )
        ...
        try:
            ...
            op.execute(
                "insert into tMetadata_new (dataset_id, key, value) select dataset_id, metadata_name, metadata_val from tMetadata"
            )
    ...
    if op.get_bind().dialect.name == "mysql":
        ...
        try:
            ...
            op.alter_column(
                "tMetadata",
                "value",
                new_column_name="metadata_val",
                existing_type=sa.Text().with_variant(mysql.TEXT(4294967295), "mysql"),
                nullable=False,
            )
        except Exception as e:
            log.exception(e)
        ...
        try:
            ...
            op.execute(
                "insert into tDataset_new (dataset_id, data_name, data_type, data_units, data_hash, value, cr_date, created_by) select id, name, type, unit, hash, value, cr_date, created_by from tDataset"
            )
        ...
        try:
            ...
            op.create_table(
                ...
                sa.Column("dataset_id", sa.Integer, primary_key=True, index=True),
                sa.Column(
                    "metadata_name", sa.String(60), nullable=False, primary_key=True
                ),
                sa.Column("metadata_val", sa.Text(), nullable=False),
            )

            op.execute(
                "insert into tMetadata_new (dataset_id, metadata_name, metadata_val) select dataset_id, key, value from tMetadata"
            )
