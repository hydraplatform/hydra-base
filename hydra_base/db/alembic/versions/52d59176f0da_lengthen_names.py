    ...
    if op.get_bind().dialect.name == "mysql":
        ...
        try:
            op.create_table(
                ...
                sa.Column(
                    "layout",
                    sa.Text().with_variant(sa.mysql.TEXT(4294967295), "mysql"),
                    nullable=True,
                ),
            )

            op.execute(
                "insert into tTemplate_new (id, name, description, cr_date, layout) select id, name, '', cr_date, layout from tTemplate"
            )
...
def downgrade():
    if op.get_bind().dialect.name == "mysql":
        ...
        try:
            ...
            op.execute(
                "insert into tTemplate_new (id, name, cr_date, layout) select id, name, cr_date, layout from tTemplate"
            )
