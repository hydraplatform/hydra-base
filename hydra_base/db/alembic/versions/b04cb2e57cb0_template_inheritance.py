...
def upgrade():
    if op.get_bind().dialect.name == "mysql":
        ...
        try:
            op.execute(
                "alter table tTypeAttr add column id int primary key auto_increment;"
            )
        except Exception as e:
            log.critical(e)
        try:
        ...
        try:
            op.create_foreign_key(
                "tTypeAttr_ibfk_1", "tTypeAttr", "tAttr", ["attr_id"], ["id"]
            )
        except Exception as e:
            log.critical(e)
        try:
            op.create_foreign_key(
                "tTypeAttr_ibfk_2", "tTypeAttr", "tTemplateType", ["type_id"], ["id"]
            )
        except Exception as e:
            log.critical(e)
        try:
