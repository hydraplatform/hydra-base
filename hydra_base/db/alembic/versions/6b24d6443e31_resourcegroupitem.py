    ...
    if op.get_bind().dialect.name == "mysql":
        ...
        try:
            ...
            op.create_table(
                ...
                sa.Column(
                    "subgroup_id", sa.Integer(), sa.ForeignKey("tResourceGroup.id")
                ),
                sa.Column(
                    "cr_date",
                    sa.TIMESTAMP(),
                ...
                sa.UniqueConstraint(
                    "group_id", "node_id", "scenario_id", name="node_group_1"
                ),
                sa.UniqueConstraint(
                    "group_id", "link_id", "scenario_id", name="link_group_1"
                ),
                sa.UniqueConstraint(
            ...
            op.execute(
                "insert into tResourceGroupItem_new (id, group_id, scenario_id, ref_key, node_id, link_id, subgroup_id, cr_date) select item_id, group_id, scenario_id, ref_key, node_id, link_id, subgroup_id, cr_date from tResourceGroupItem"
            )
...
def downgrade():
    if op.get_bind().dialect.name == "mysql":
        ...
        try:
            ...
            op.execute(
                "insert into tResourceGroupItem_new (item_id, group_id, scenario_id, ref_key, node_id, link_id, subgroup_id, cr_date) select id, group_id, scenario_id, ref_key, node_id, link_id, subgroup_id, cr_date from tResourceGroupItem"
            )
