...
def upgrade():
    if op.get_bind().dialect.name == "mysql":
        ...
        try:
            op.alter_column(
                "tUser", "password", existing_type=sa.LargeBinary(), nullable=False
            )
        ...
        try:
            ...
            op.execute(
                "insert into tUser_new (id, username, password, display_name, last_login, last_edit, cr_date) select id, username, password, display_name, last_login, last_edit, cr_date from tUser"
            )
...
def downgrade():
    if op.get_bind().dialect.name == "mysql":
        ...
        try:
            op.alter_column(
                "tUser", "password", existing_type=sa.Text(), nullable=False
            )
        except Exception as e:
            log.exception(e)
