...
def truncate_all_audit_tables():
    ...
    for t in tables:
        ...
        if audit_table is None:
            ...
            continue
        export_table_to_sqlite(
            mysql_session, sqlite_session, sqlite_metadata, audit_table
        )
...
def truncate_table(session, table, audit_table):
    ...
    for r in rs:
        ...
        rs = (
            session.query(aud_id_col)
            .filter(and_(*args))
            .order_by(aud_id_col.desc())[3:]
        )
...
def export_table_to_sqlite(mysql_session, sqlite_session, sqlite_metadata, audit_table):
    ...
    if len(new_data) > 0:
        ...
        ins = ins.format(
            tablename=sqlite_table.name, colnames=colnames, markers=markers
        )
